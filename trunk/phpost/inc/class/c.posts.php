...
class tsPosts {
	...
	*/
	function newPost(){
        ...
        //ANTIFLOOD
        $antiflood = 2;
        $d = mysql_fetch_row(mysql_query('SELECT COUNT(post_id) AS few FROM `p_posts` WHERE post_body = \''.$postData['body'].'\' LIMIT 1'));
		...
		if($tsUser->info['user_lastpost'] < (time() - $antiflood)) {
            // EXISTE LA CATEGORIA?
			$query = mysql_query('SELECT cid FROM p_categorias WHERE cid = \''.(int)$postData['category'].'\' LIMIT 1');
            if(mysql_num_rows($query) == 0) return 'La categor&iacute;a especificada no existe.';
			// INSERTAMOS
			...
			if(mysql_query('INSERT INTO `p_posts` (post_user, post_category, post_title, post_body, post_date, post_tags, post_ip, post_private, post_block_comments, post_sponsored, post_sticky, post_smileys, post_visitantes, post_status) VALUES (\''.$tsUser->uid.'\', \''.(int)$postData['category'].'\', \''.$postData['title'].'\',  \''.$postData['body'].'\', \''.$postData['date'].'\', \''.$postData['tags'].'\', \''.$_SERVER['REMOTE_ADDR'].'\', \''.(int)$postData['private'].'\', \''.(int)$postData['block_comments'].'\', \''.(int)$postData['sponsored'].'\', \''.(int)$postData['sticky'].'\', \''.(int)$postData['smileys'].'\', \''.(int)$postData['visitantes'].'\', '.(!$tsUser->is_admod && ($tsCore->settings['c_desapprove_post'] == 1 || $tsUser->permisos['gorpap'] == true) ? '\'3\'' : '\'0\'').')')) {
				...
				if(!$tsUser->is_admod && ($tsCore->settings['c_desapprove_post'] == 1 || $tsUser->permisos['gorpap'] == true)) mysql_query('INSERT INTO `w_historial` (`pofid`, `action`, `type`, `mod`, `reason`, `date`, `mod_ip`) VALUES (\''.(int)$postID.'\', \'3\', \'1\', \''.$tsUser->uid.'\', \'Revisi&oacute;n al publicar\', \''.time().'\', \''.$_SERVER['REMOTE_ADDR'].'\')');
				$time = time();
                // ESTAD�STICAS
                mysql_query('UPDATE `w_stats` SET `stats_posts` = stats_posts + \'1\' WHERE `stats_no` = \'1\'');
				// ULTIMO POST
				mysql_query('UPDATE u_miembros SET user_lastpost = \''.$time.'\' WHERE user_id = \''.$tsUser->uid.'\'');
	...
	*/
	function savePost(){
		...
		$query = mysql_query('SELECT post_user, post_sponsored, post_sticky FROM p_posts WHERE post_id = \''.(int)$post_id.'\' LIMIT 1');
    ...
    */
    function setNP()
    {
        ...
        $action = $_GET['action'];
        if($action == 'fortuitae'){
        $query = mysql_query('SELECT p.post_id, p.post_user, p.post_category, p.post_title, u.user_name, c.c_nombre, c.c_seo FROM p_posts AS p LEFT JOIN u_miembros AS u ON p.post_user = u.user_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE p.post_status = \'0\' '.($tsUser->is_admod && $tsCore->settings['c_see_mod'] == 1 ? '' : 'AND u.user_activo = \'1\' && u.user_baneado = \'0\'').' ORDER BY RAND() DESC LIMIT 1') or exit(mysql_error());
        ...
        $query = mysql_query('SELECT p.post_id, p.post_user, p.post_category, p.post_title, u.user_name, c.c_nombre, c.c_seo FROM p_posts AS p LEFT JOIN u_miembros AS u ON p.post_user = u.user_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE p.post_status = \'0\' '.($tsUser->is_admod && $tsCore->settings['c_see_mod'] == 1 ? '' : 'AND u.user_activo = \'1\' && u.user_baneado = \'0\'').' AND p.post_id '.$action.' '.(int)$pid.' ORDER BY p.post_id '.($action == '<' ? 'DESC' : 'ASC').' LIMIT 1') or exit(mysql_error());
    	if(!mysql_num_rows($query)){
    ...
    */
    function getCatData(){
        ...
        $cat = intval($_GET['cat']);
        //
		$query = mysql_query('SELECT c_nombre, c_seo FROM p_categorias WHERE c_seo = \''.(int)$cat.'\' LIMIT 1');
        $data = mysql_fetch_assoc($query);
	...
	*/
    function getLastPosts($category = NULL, $subcateg = NULL, $sticky = false)
    {
      ...
      if($sticky) 
      {
       ...
       $s_order = 'p.post_id';
       // TOTAL DE POSTS
       $q1 = mysql_fetch_row(mysql_query('SELECT COUNT(p.post_id) AS total FROM p_posts AS p LEFT JOIN u_miembros AS u ON p.post_user = u.user_id WHERE '.($tsUser->is_admod && $tsCore->settings['c_see_mod'] == 1 ? ' p.post_id > \'0\' ' : ' u.user_activo = \'1\' && u.user_baneado = \'0\' && p.post_status = \'0\'').' '.$p_where.' '.$s_where));
       $posts['total'] = $q1[0];
                            //
	...
	*/
	function getPost(){
		...
		if(empty($postData['post_id'])) {
			$tsDraft = mysql_fetch_assoc(mysql_query('SELECT b_title FROM p_borradores WHERE b_post_id = \''.(int)$post_id.'\' LIMIT 1'));
		...
		$q1 = mysql_fetch_row(mysql_query('SELECT COUNT(u.user_name) AS c FROM u_miembros AS u LEFT JOIN p_comentarios AS c ON u.user_id = c.c_user WHERE c.c_post_id = \''.(int)$post_id.'\' && c.c_status = \'0\' && u.user_activo = \'1\' && u.user_baneado = \'0\''));
		$q2 = mysql_fetch_row(mysql_query('SELECT COUNT(u.user_name) AS s FROM u_miembros AS u LEFT JOIN u_follows AS f ON u.user_id = f.f_user WHERE f.f_type = \'2\' && f.f_id = \''.(int)$post_id.'\' && u.user_activo = \'1\' && u.user_baneado = \'0\''));
		$q3 = mysql_fetch_row(mysql_query('SELECT COUNT(follow_id) AS m FROM u_follows WHERE f_type = \'3\' && f_id = \''.(int)$post_id.'\''));
		$q4 = mysql_fetch_row(mysql_query('SELECT COUNT(fav_id) AS f FROM p_favoritos WHERE fav_post_id = \''.(int)$post_id.'\''));
        ...
        mysql_query('UPDATE p_posts SET post_comments = \''.$q1[0].'\', post_seguidores = \''.$q2[0].'\', post_shared = \''.$q3[0].'\', post_favoritos = \''.$q4[0].'\', post_cache = \''.time().'\' WHERE post_id = \''.(int)$post_id.'\'');
        }
        
        // BLOQUEADO
		$query = mysql_query('SELECT bid FROM u_bloqueos WHERE b_user = \''.(int)$postData['post_user'].'\' AND b_auser = \''.$tsUser->uid.'\' LIMIT 1');
		$postData['block'] = mysql_num_rows($query);
		
		// FOLLOWS
		if($postData['post_seguidores'] > 0){
			$q1 = mysql_fetch_row(mysql_query('SELECT COUNT(follow_id) AS f FROM u_follows WHERE f_id = \''.(int)$postData['post_id'].'\' AND f_user = \''.$tsUser->uid.'\' AND f_type = \'2\''));
			$postData['follow'] = $q1[0];	
		}
		//VISITANTES RECIENTES
		if($postData['post_visitantes']){
		$postData['visitas'] = result_array(mysql_query('SELECT v.*, u.user_id, u.user_name FROM w_visitas AS v LEFT JOIN u_miembros AS u ON v.user = u.user_id WHERE v.for = \''.(int)$postData['post_id'].'\' && v.type = \'2\' && v.user > 0 ORDER BY v.date LIMIT 10'));
		}
        //PUNTOS
        if($postData['post_user'] == $tsUser->uid || $tsUser->is_admod){
        $postData['puntos'] = result_array(mysql_query('SELECT p.*, u.user_id, u.user_name FROM p_votos AS p LEFT JOIN u_miembros AS u ON p.tuser = u.user_id WHERE p.tid = \''.(int)$postData['post_id'].'\' && p.type = \'1\' ORDER BY p.cant DESC'));
		}
        // CATEGORIAS
		...
		// MEDALLAS
        $query = mysql_query('SELECT m.*, a.* FROM w_medallas AS m LEFT JOIN w_medallas_assign AS a ON a.medal_id = m.medal_id WHERE a.medal_for = \''.(int)$postData['post_id'].'\' AND m.m_type = \'2\' ORDER BY a.medal_date');
	   ...
	   // FECHA
		...
		$visitado = mysql_num_rows(mysql_query('SELECT id FROM `w_visitas` WHERE `for` = \''.(int)$post_id.'\' && `type` = \'2\' && '.($tsUser->is_member ? '(`user` = \''.$tsUser->uid.'\' OR `ip` LIKE \''.$_SERVER['REMOTE_ADDR'].'\')' : '`ip` LIKE \''.$_SERVER['REMOTE_ADDR'].'\'').' LIMIT 1'));
		if($tsUser->is_member && $visitado == 0) {
			mysql_query('INSERT INTO w_visitas (`user`, `for`, `type`, `date`, `ip`) VALUES (\''.$tsUser->uid.'\', \''.(int)$post_id.'\', \'2\', \''.time().'\', \''.$_SERVER['REMOTE_ADDR'].'\')');
			mysql_query('UPDATE p_posts SET post_hits = post_hits + 1 WHERE post_id = \''.(int)$post_id.'\' AND post_user != \''.$tsUser->uid.'\'');
		}else{
		mysql_query('UPDATE `w_visitas` SET `date` = \''.time().'\', ip = \''.$_SERVER['REMOTE_ADDR'].'\' WHERE `for` = \''.(int)$post_id.'\' && `type` = \'2\'');
		}
		if($tsCore->settings['c_hits_guest'] == 1 && !$tsUser->is_member && !$visitado) {
			mysql_query('INSERT INTO w_visitas (`user`, `for`, `type`, `date`, `ip`) VALUES (\''.$tsUser->uid.'\', \''.(int)$post_id.'\', \'2\', \''.time().'\', \''.$_SERVER['REMOTE_ADDR'].'\')');
			mysql_query('UPDATE p_posts SET post_hits = post_hits + 1 WHERE post_id = \''.(int)$post_id.'\'');
		}
        // AGREGAMOS A VISITADOS... PORTAL
        if($tsCore->settings['c_allow_portal']){
		    $query = mysql_query('SELECT last_posts_visited FROM u_portal WHERE user_id = \''.$tsUser->uid.'\' LIMIT 1');
            $data = mysql_fetch_assoc($query);
			...
			mysql_query('UPDATE u_portal SET last_posts_visited = \''.$visited.'\' WHERE user_id = \''.$tsUser->uid.'\'');
        }
		//
		return $postData;
	}
	...
	*/
	function getAutor($user_id){
	   global $tsUser, $tsCore;
		...
		$data['user_comentarios'] = mysql_num_rows(mysql_query('SELECT cid FROM p_comentarios WHERE c_user = \''.(int)$user_id.'\' && c_status = \'0\''));
		$data['user_posts'] = mysql_num_rows(mysql_query('SELECT post_id FROM p_posts WHERE post_user = \''.(int)$user_id.'\' && post_status = \'0\''));
		
		// RANGOS DE ESTE USUARIO
		$query = mysql_query('SELECT r_name, r_color, r_image FROM u_rangos WHERE rango_id = \''.$data['user_rango'].'\' LIMIT 1');
		...
		include(TS_EXTRA."datos.php");
                //fix Warning produced by: include("../ext/datos.php");
		$data['pais'] = array('icon' => strtolower($data['user_pais']),'name' => $tsPaises[$data['user_pais']]);
		// FOLLOWS
		if($data['user_seguidores'] > 0){
			$query = mysql_query('SELECT follow_id FROM u_follows WHERE f_id = \''.(int)$user_id.'\' AND f_user = \''.$tsUser->uid.'\' AND f_type = \'1\'');
	...
	*/
	function getEditPost(){
		...
		$pid = intval($_GET['pid']);
		//
		$query = mysql_query('SELECT * FROM p_posts WHERE post_id = \''.(int)$pid.'\' LIMIT 1');
		$ford = mysql_fetch_assoc($query);
		
        //
        if(empty($ford['post_id'])) return 'El post elegido no existe.';
        elseif(($tsUser->uid != $ford['post_user']) && ($tsUser->is_admod == 0 && $tsUser->permisos['moedpo'] == false)) return 'No puedes editar un post que no es tuyo.';
		// PEQUE�O HACK
	...
	*/
	function deletePost(){
		...
		$query = mysql_query('SELECT post_id, post_title, post_user, post_body, post_category FROM p_posts WHERE post_id = \''.(int)$post_id.'\' AND post_user = \''.$tsUser->uid.'\'');
		$data = mysql_fetch_assoc($query);
		
        mysql_query('UPDATE `w_stats` SET `stats_posts` = stats_posts - \'1\' WHERE `stats_no` = \'1\'');
        mysql_query('UPDATE `u_miembros` SET `user_posts` = user_posts - \'1\' WHERE `user_id` = \''.$data['post_user'].'\'');
		...
		if(!empty($data['post_id']) || !empty($tsUser->is_admod)){
			...
			if(mysql_query('DELETE FROM p_posts WHERE post_id = \''.(int)$post_id.'\'')) {
				if(mysql_query('DELETE FROM p_comentarios WHERE c_post_id = \''.(int)$post_id.'\'')) {
                   if(mysql_query('INSERT INTO `p_borradores` (b_user, b_date, b_title, b_body, b_tags, b_category, b_status, b_causa) VALUES (\''.$tsUser->uid.'\', \''.time().'\', \''.$tsCore->setSecure($data['post_title']).'\', \''.$tsCore->setSecure($data['post_body']).'\', \'\', \''.$data['post_category'].'\', \'2\', \'\')'))
                    return "1: El post fue eliminado satisfactoriamente.";  
                 }
			}else {
			    if(mysql_query('UPDATE p_posts SET post_status = \'2\' WHERE post_id = \''.(int)$post_id.'\'')) return "1: El post se ha eliminado correctamente.";
			}
	...
	function deleteAdminPost(){
		global $tsUser;
		     if($tsUser->is_admod == 1){
			    if(mysql_num_rows(mysql_query('SELECT post_id FROM p_posts WHERE post_id = \''.(int)$_POST['postid'].'\' AND post_status = \'2\''))){
				 if(mysql_query('DELETE FROM p_posts WHERE post_id = \''.(int)$_POST['postid'].'\'')) {
	           if(mysql_query('DELETE FROM p_comentarios WHERE c_post_id = \''.(int)$_POST['postid'].'\' ')){
	               mysql_query('UPDATE `w_stats` SET `stats_posts` = stats_posts - \'1\' WHERE `stats_no` = \'1\'');
	...
	*/
	function getRelated($tags){
		...
		//
		$query = mysql_query("SELECT DISTINCT p.post_id, p.post_title, p.post_category, p.post_private, c.c_seo, c.c_img FROM p_posts AS p LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE MATCH (post_tags) AGAINST ('$tags' IN BOOLEAN MODE) AND p.post_status = 0 AND post_sticky = 0 ORDER BY rand() LIMIT 0,10");
		//
		$data = result_array($query);
	...
	*/
	function getComentarios($post_id){
		...
		$query = mysql_query('SELECT u.user_name, u.user_activo, u.user_baneado, c.* FROM u_miembros AS u LEFT JOIN p_comentarios AS c ON u.user_id = c.c_user WHERE c.c_post_id = \''.(int)$post_id.'\' '.($tsUser->is_admod ? '' : 'AND c.c_status = \'0\' AND u.user_activo = \'1\' && u.user_baneado = \'0\'').' ORDER BY c.cid LIMIT '.$start);
		// COMENTARIOS TOTALES
		$return['num'] = mysql_num_rows(mysql_query('SELECT cid FROM p_comentarios WHERE c_post_id = \''.(int)$post_id.'\' '.($tsUser->is_admod ? '' : 'AND c_status = \'0\'').''));
		//
		...
		foreach($comments as $comment)
			...
			if($comment['c_votos'] != 0){
			    $query = mysql_query('SELECT voto_id FROM p_votos WHERE tid = \''.(int)$comment['cid'].'\' AND tuser = \''.$tsUser->uid.'\' AND type = \'2\' LIMIT 1');
				$votado = mysql_num_rows($query);
				
			} else $votado = 0;
			
			// BLOQUEADO
		    $return['block'] = mysql_num_rows(mysql_query('SELECT bid, b_user, b_auser FROM `u_bloqueos` WHERE b_user = \''.(int)$comment['c_user'].'\' AND b_auser = \''.$tsUser->uid.'\' LIMIT 1'));
			...
			$i++;
                ...
                } // Fix Warning foreach Invalid Argument, una pavada
		//
		return $return;
	}
	/*
	...
	*/
	function newComentario(){
		...
		/* DE QUIEN ES EL POST */
		$query = mysql_query('SELECT post_user, post_block_comments FROM p_posts WHERE post_id = \''.(int)$post_id.'\' LIMIT 1');
		$data = mysql_fetch_assoc($query);
        ...
        if($data['post_user']){
            if($data['post_block_comments'] != 1 || $data['post_user'] == $tsUser->uid || $tsUser->is_admod || $tsUser->permisos['mocepc']){
				...
				$_SERVER['REMOTE_ADDR'] = $_SERVER['X_FORWARDED_FOR'] ? $_SERVER['X_FORWARDED_FOR'] : $_SERVER['REMOTE_ADDR'];
                if(!filter_var($_SERVER['REMOTE_ADDR'], FILTER_VALIDATE_IP)) { die('0: Su ip no se pudo validar.'); }
				if(mysql_query('INSERT INTO `p_comentarios` (`c_post_id`, `c_user`, `c_date`, `c_body`, `c_ip`) VALUES (\''.(int)$post_id.'\', \''.$tsUser->uid.'\', \''.$fecha.'\', \''.$comentario.'\', \''.$_SERVER['REMOTE_ADDR'].'\')')) {
        		  	$cid = mysql_insert_id();
                    //SUMAMOS A LAS ESTAD�STICAS
                    mysql_query('UPDATE w_stats SET stats_comments = stats_comments + 1 WHERE stats_no = \'1\'');
                    mysql_query('UPDATE p_posts SET post_comments = post_comments +  1 WHERE post_id = \''.(int)$post_id.'\'');
                    mysql_query('UPDATE u_miembros SET user_comentarios = user_comentarios + 1 WHERE user_id = \''.$tsUser->uid.'\'');
    ...
    */
    function editComentario(){
        ...
        if($tsText == '') return '0: El campo <b>Comentario</b> es requerido para esta operaci&oacute;n';
        //
		$query = mysql_query('SELECT c_user FROM p_comentarios WHERE cid = \''.$cid.'\' LIMIT 1');
        $cuser = mysql_fetch_assoc($query);
        
        //
        if($tsUser->is_admod || ($tsUser->uid == $cuser['c_user'] && $tsUser->permisos['goepc']) || $tsUser->permisos['moedcopo']){
            // ANTI FLOOD
            $tsCore->antiFlood();
			if(mysql_query('UPDATE p_comentarios SET c_body = \''.$comentario.'\' WHERE cid = \''.(int)$cid.'\''))
	...
	*/
	function delComentario(){
		...
		if(!mysql_num_rows(mysql_query('SELECT cid FROM p_comentarios WHERE cid = \''.(int)$comid.'\''))){ return '0: El comentario no existe'; } // [17/04/2012] Evitar miles de comentarios
		$query = mysql_query('SELECT post_id FROM p_posts WHERE post_id = \''.(int)$post_id.'\' AND post_user = \''.$tsUser->uid.'\'');
		$is_mypost = mysql_num_rows($query);
		
		// ES MI COMENTARIO?
		$query = mysql_query('SELECT cid FROM p_comentarios WHERE cid = \''.(int)$comid.'\' AND c_user = \''.$tsUser->uid.'\'');
		$is_mycmt = mysql_num_rows($query);
		
		// SI ES....
		if(!empty($is_mypost) || (!empty($is_mycmt) && !empty($tsUser->permisos['godpc'])) || !empty($tsUser->is_admod) || !empty($tsUser->permisos['moecp'])){
		    if(mysql_query('DELETE FROM p_comentarios WHERE cid = \''.(int)$comid.'\' AND c_user = \''.(int)$autor.'\' AND c_post_id = \''.(int)$post_id.'\'')) {
				// BORRAR LOS VOTOS
				mysql_query('DELETE FROM p_votos WHERE tid = \''.(int)$comid.'\'');
                // RESTAR EN LAS ESTAD�STICAS
                mysql_query('UPDATE w_stats SET stats_comments = stats_comments - 1 WHERE stats_no = \'1\'');
                mysql_query('UPDATE p_posts SET post_comments = post_comments - 1 WHERE post_id = \''.(int)$post_id.'\'');
                mysql_query('UPDATE u_miembros SET user_comentarios = user_comentarios - 1 WHERE user_id = \''.(int)$autor.'\'');
				//
	...
	*/
	function OcultarComentario(){
		...
		//
		$data = mysql_fetch_assoc($query = mysql_query('SELECT cid, c_user, c_post_id, c_status FROM p_comentarios WHERE cid = \''.(int)$_POST['comid'].'\''));
		mysql_query('UPDATE w_stats SET stats_comments = stats_comments '.($data['c_status'] == 1 ? '+' : '-').' 1 WHERE stats_no = \'1\'');
        mysql_query('UPDATE p_posts SET post_comments = post_comments '.($data['c_status'] == 1 ? '+' : '-').' 1 WHERE post_id = \''.$data['c_post_id'].'\'');
        mysql_query('UPDATE u_miembros SET user_comentarios = user_comentarios '.($data['c_status'] == 1 ? '+' : '-').' 1 WHERE user_id = \''.$data['c_user'].'\'');
		// OCULTAMOS O MOSTRAMOS
		if(mysql_query('UPDATE p_comentarios SET c_status = '.($data['c_status'] == 1 ? '\'0\'' : '\'1\'').' WHERE cid = \''.(int)$_POST['comid'].'\'')) {
	...
	*/
	function votarComentario(){
		...
		//
		$query = mysql_query('SELECT c_user FROM p_comentarios WHERE cid = \''.(int)$cid.'\' LIMIT 1');
		$data = mysql_fetch_assoc($query);
		...
		if(!$is_mypost){
			// YA LO VOTE?
			$query = mysql_query('SELECT tid FROM p_votos WHERE tid = \''.(int)$cid.'\' AND tuser = \''.$tsUser->uid.'\' AND type = \'2\' LIMIT 1');
			$votado = mysql_num_rows($query);
			
			if(empty($votado)){
				// SUMAR VOTO
				mysql_query('UPDATE p_comentarios SET c_votos = c_votos '.$voto.' WHERE cid = \''.(int)$cid.'\'');
				// INSERTAR EN TABLA
				if(mysql_query('INSERT INTO p_votos (tid, tuser, type) VALUES (\''.(int)$cid.'\', \''.$tsUser->uid.'\', \'2\' ) ')){
					// SUMAR PUNTOS??
					if($votoVal == 1 && $tsCore->settings['c_allow_sump'] == 1) {
					    mysql_query('UPDATE u_miembros SET user_puntos = user_puntos +1 WHERE user_id = \''.$data['c_user'].'\'');
	...
	*/
	function votarPost(){
		...
		if($tsUser->is_admod != 1){
		if(mysql_num_rows(mysql_query('SELECT user_id FROM u_miembros WHERE user_last_ip =  \''.$_SERVER['REMOTE_ADDR'].'\' AND user_id != \''.$tsUser->uid.'\'')) || mysql_num_rows(mysql_query('SELECT session_id FROM u_sessions WHERE session_ip =  \''.$tsCore->setSecure($_SERVER['REMOTE_ADDR']).'\' AND session_user_id != \''.$tsUser->uid.'\''))) return '0: Has usado otra cuenta anteriormente, deber&aacute;s contactar con la administraci&oacute;n.';
		}
		...
		$max_points = $tsUser->permisos['gopfp'];
				...
				if($puntos <= $max_points) { // seroo churra XD ._. No alteraciones de javascript para sumar m�s de lo que se permite (? LOL ��
					...
					mysql_query('UPDATE u_miembros SET user_puntosxdar = user_puntosxdar - \''.(int)$puntos.'\' WHERE user_id = \''.$tsUser->uid.'\'');
					// INSERTAR EN TABLA
					mysql_query('INSERT INTO p_votos (tid, tuser, cant, type, date) VALUES (\''.(int)$post_id.'\', \''.$tsUser->uid.'\', \''.(int)$puntos.'\', \'1\', \''.time().'\')');
					// AGREGAR AL MONITOR
			   ...
			   } else return '0: Voto no v&aacute;lido. No puedes no dar puntos (lo quie se define como *****).';
	...
	*/
	function saveFavorito(){
		...
		if($data['post_user'] != $tsUser->uid){
			// YA LO TENGO?
			$my_favorito = mysql_num_rows(mysql_query('SELECT fav_id FROM p_favoritos WHERE fav_post_id = \''.(int)$post_id.'\' AND fav_user = \''.$tsUser->uid.'\' LIMIT 1'));
			if(empty($my_favorito)){
			    if(mysql_query('INSERT INTO p_favoritos (fav_user, fav_post_id, fav_date) VALUES (\''.$tsUser->uid.'\', \''.(int)$post_id.'\', \''.$fecha.'\')')) {
					// AGREGAR AL MONITOR
	...
	*/
	function getFavoritos(){
		global $tsCore, $tsUser;
		//
		$query = mysql_query('SELECT f.fav_id, f.fav_date, p.post_id, p.post_title, p.post_date, p.post_puntos, COUNT(p_c.c_post_id) as post_comments,  c.c_nombre, c.c_seo, c.c_img FROM p_favoritos AS f LEFT JOIN p_posts AS p ON p.post_id = f.fav_post_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category LEFT JOIN p_comentarios AS p_c ON p.post_id = p_c.c_post_id && p_c.c_status = \'0\' WHERE f.fav_user = \''.$tsUser->uid.'\' AND p.post_status = \'0\' GROUP BY c_post_id');
		$data = result_array($query);
	...
	*/
	function delFavorito(){
		...
		$query = mysql_query('SELECT fav_post_id FROM p_favoritos WHERE fav_id = \''.(int)$fav_id.'\' AND fav_user = \''.$tsUser->uid.'\' LIMIT 1');
		$data = mysql_fetch_assoc($query);
		$is_myfav = mysql_num_rows($query);
		
		// ES MI FAVORITO?
		if(!empty($data['fav_post_id'])){
		    if(mysql_query('DELETE FROM p_favoritos WHERE fav_id = \''.(int)$fav_id.'\' AND fav_user = \''.$tsUser->uid.'\'')){
	...
	*/
	function subirRango($user_id, $post_id = false){
		global $tsCore, $tsUser;
		// CONSULTA
        $query = mysql_query('SELECT u.user_puntos, u.user_rango, r.r_type FROM u_miembros AS u LEFT JOIN u_rangos AS r ON u.user_rango = r.rango_id WHERE u.user_id = \''.$user_id.'\' LIMIT 1');
		$data = mysql_fetch_assoc($query);
        ...
        if(!empty($post_id) && $tsCore->settings['c_newr_type'] == 0) {
		    $query = mysql_query('SELECT post_puntos FROM p_posts WHERE post_id = \''.(int)$post_id.'\' LIMIT 1');
            $puntos = mysql_fetch_assoc($query);
        ...
        $posts = mysql_fetch_row(mysql_query('SELECT COUNT(post_id) AS p FROM p_posts WHERE post_user = \''.(int)$user_id.'\' && post_status = \'0\''));
		$fotos = mysql_fetch_row(mysql_query('SELECT COUNT(foto_id) AS f FROM f_fotos WHERE f_user = \''.(int)$user_id.'\' && f_status = \'0\''));
        $comentarios = mysql_fetch_row(mysql_query('SELECT COUNT(cid) AS c FROM p_comentarios WHERE c_user = \''.(int)$user_id.'\' && c_status = \'0\''));
        
        $rango_actual = $data['user_rango'];
        
		// RANGOS
		$query = mysql_query('SELECT rango_id, r_cant, r_type FROM u_rangos WHERE rango_id != \''.$rango_actual.'\' AND r_type > \'0\' ORDER BY r_cant');
		$data = result_array($query);
		...
		if(!empty($newRango)){
			//
			if(mysql_query('UPDATE u_miembros SET user_rango = \''.$newRango.'\' WHERE user_id = \''.$user_id.'\'')) return true;
		}
	}
	...
	*/
	function DarMedalla($post_id){
		//
		$data = mysql_fetch_assoc($query = mysql_query('SELECT post_id, post_user, post_puntos, post_hits FROM p_posts WHERE post_id = \''.(int)$post_id.'\' LIMIT 1'));
        
		#���#
        $q1 = mysql_fetch_row(mysql_query('SELECT COUNT(follow_id) AS se FROM u_follows WHERE f_id = \''.(int)$post_id.'\' && f_type = \'2\''));
        $q2 = mysql_fetch_row(mysql_query('SELECT COUNT(cid) AS c FROM p_comentarios WHERE c_post_id = \''.(int)$post_id.'\' && c_status = \'0\''));
        $q3 = mysql_fetch_row(mysql_query('SELECT COUNT(fav_id) AS f FROM p_favoritos WHERE fav_post_id = \''.(int)$post_id.'\''));
        $q4 = mysql_fetch_row(mysql_query('SELECT COUNT(did) AS d FROM w_denuncias WHERE obj_id = \''.(int)$post_id.'\' && d_type = \'1\''));
        $q5 = mysql_fetch_row(mysql_query('SELECT COUNT(wm.medal_id) AS m FROM w_medallas AS wm LEFT JOIN w_medallas_assign AS wma ON wm.medal_id = wma.medal_id WHERE wm.m_type = \'2\' AND wma.medal_for = \''.(int)$post_id.'\''));
        $q6 = mysql_fetch_row(mysql_query('SELECT COUNT(follow_id) AS sh FROM u_follows WHERE f_id = \''.(int)$post_id.'\' && f_type = \'3\''));
		// MEDALLAS
		$datamedal = result_array($query = mysql_query('SELECT medal_id, m_cant, m_cond_post FROM w_medallas WHERE m_type = \'2\' ORDER BY m_cant DESC'));
		...
		if(!empty($newmedalla)){
		if(!mysql_num_rows(mysql_query('SELECT id FROM w_medallas_assign WHERE medal_id = \''.(int)$newmedalla.'\' AND medal_for = \''.(int)$post_id.'\''))){
		mysql_query('INSERT INTO `w_medallas_assign` (`medal_id`, `medal_for`, `medal_date`, `medal_ip`) VALUES (\''.(int)$newmedalla.'\', \''.(int)$post_id.'\', \''.time().'\', \''.$_SERVER['REMOTE_ADDR'].'\')');
		mysql_query('INSERT INTO u_monitor (user_id, obj_uno, obj_dos, not_type, not_date) VALUES (\''.(int)$data['post_user'].'\', \''.(int)$newmedalla.'\', \''.(int)$post_id.'\', \'16\', \''.time().'\')'); 
		mysql_query('UPDATE w_medallas SET m_total = m_total + 1 WHERE medal_id = \''.(int)$newmedalla.'\'');}
		}
            } // end foreach correct bug	
          } // end if fix warning before
	}
    /*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*\
								BUSCADOR
	/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
    ...
    */
    function getQuery()
    {
        ...
        // PAGINAS
        $query = mysql_query('SELECT COUNT(p.post_id) AS total FROM p_posts AS p WHERE p.post_status = \'0\' '.$where_cat.' '.$w_autor.' '.$w_search.' ORDER BY p.post_date');
        $total = mysql_fetch_assoc($query);
        $total = $total['total'];
        
        $data['pages'] = $tsCore->getPagination($total, 12);
        //
        $query = mysql_query('SELECT p.post_id, p.post_user, p.post_category, p.post_title, p.post_date, p.post_comments, p.post_favoritos, p.post_puntos, u.user_name, c.c_seo, c.c_nombre, c.c_img FROM p_posts AS p LEFT JOIN u_miembros AS u ON u.user_id = p.post_user LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE p.post_status = \'0\' '.$where_cat.' '.$w_autor.' '.$w_search.' ORDER BY p.post_date DESC LIMIT '.$data['pages']['limit']);
