...
class tsMonitor {
	...
	*/
	public function __construct(){
		...
		// NOTIFICACIONES
		$query = mysql_query('SELECT COUNT(not_id) AS total FROM u_monitor WHERE user_id = \''.$tsUser->uid.'\' AND not_menubar > 0');
         ...
         * AVISOS
        */ 
		$query = mysql_query('SELECT COUNT(av_id) AS total FROM u_avisos WHERE user_id = \''.$tsUser->uid.'\' AND av_read = \'0\'');
		$data = mysql_fetch_assoc($query);
    ...
    */
	...
	global $tsCore;
		...
		$query = mysql_query('SELECT user_baneado FROM u_miembros WHERE user_id = \''.(int)$user_id.'\' LIMIT 1');
        $data = mysql_fetch_assoc($query);
        
        # NO PODEMOS ENVIAR A UN USUARIO BANEADO
        if($data['user_baneado'] == 1) return true;
        # INSERTAMOS EL AVISO
		if(mysql_query('INSERT INTO u_avisos (user_id, av_subject, av_body, av_date, av_type) VALUES (\''.(int)$user_id.'\', \''.$tsCore->setSecure($subject).'\', \''.$tsCore->setSecure($body).'\', \''.time().'\', \''.$type.'\' )')) return true;
        else return false;
    }
    /**
     * @name getAvisos
    ...
    public function getAvisos(){
        ...
        global $tsUser;
        # OBTENERMOS
		$query = mysql_query('SELECT * FROM u_avisos WHERE user_id = \''.$tsUser->uid.'\'');
    ...
    public function readAviso($av_id){
        ...
        global $tsUser;
        # OBTENEMOS
		$query = mysql_query('SELECT * FROM u_avisos WHERE av_id = \''.(int)$av_id.'\'');
		$data = mysql_fetch_assoc($query);
		
        # RETURN
        if(empty($data['av_id']) || $data['user_id'] != $tsUser->uid && !$tsUser->is_admod == 1) return 'El aviso no existe';
        else {
		    mysql_query('UPDATE u_avisos SET av_read = 1 WHERE av_id = \''.(int)$av_id.'\'');
    ...
    public function delAviso($av_id){
        ...
        global $tsUser;
        # OBTENEMOS
		$query = mysql_query('SELECT user_id FROM u_avisos WHERE av_id = \''.(int)$av_id.'\'');
		$data = mysql_fetch_assoc($query);
		
        # RETURN
        if(empty($data['user_id']) || $data['user_id'] != $tsUser->uid && !$tsUser->is_admod == 1) return false;
        else {
		    mysql_query('DELETE FROM u_avisos WHERE av_id = \''.(int)$av_id.'\'');
            return true;
        }
		}
	/**
...
public function setNotificacion($type, $user_id, $obj_user, $obj_uno = 0, $obj_dos = 0, $obj_tres = 0){
		...
		if($user_id != $tsUser->uid){
			...
			$query = mysql_query('SELECT not_id FROM u_monitor WHERE user_id = \''.(int)$user_id.'\' AND obj_uno = \''.(int)$obj_uno.'\' AND obj_dos = \''.(int)$obj_dos.'\' AND not_type = \''.(int)$type.'\' AND not_date > \''.$tiempo.'\' AND not_menubar > \'0\' ORDER BY not_id DESC LIMIT 1');            
			$not_data = mysql_fetch_assoc($query);
			...
			$query = mysql_query('SELECT not_id FROM u_monitor WHERE user_id = \''.(int)$user_id.'\' ORDER BY not_id DESC');
            $data = result_array($query);
			...
			if($ntotal > $tsCore->settings['c_max_nots']){
			     mysql_query('DELETE FROM u_monitor WHERE not_id = \''.(int)$delid.'\'');
			}
            // ACTUALIZAMOS / INSERTAMOS
            if($not_db_type == 'update'){
                if(mysql_query('UPDATE u_monitor SET obj_user = \''.(int)$obj_user.'\', not_date = \''.time().'\', not_total = not_total + 1 WHERE not_id = \''.(int)$not_data['not_id'].'\''))
                return true;
            } else {
                if(mysql_query('INSERT INTO u_monitor (user_id, obj_user, obj_uno, obj_dos, obj_tres, not_type, not_date) VALUES (\''.$user_id.'\', \''.$obj_user.'\', \''.$obj_uno.'\', \''.$obj_dos.'\', \''.$obj_tres.'\', \''.$type.'\', \''.time().'\')'))
                return true;   
            }
		}
	}
	...
	*/
	function setFollowNotificacion($notType, $f_type, $user_id, $obj_uno, $obj_dos = 0, $excluir){
		...
		$query = mysql_query('SELECT f_user FROM u_follows WHERE f_id = \''.(int)$f_id.'\' AND f_type = \''.(int)$f_type.'\'');
		$data = result_array($query);
    ...
    public function setMuroRepost($pub_id, $p_user, $p_user_pub){
       global $tsUser;
        //
		$query = mysql_query('SELECT c_user FROM u_muro_comentarios WHERE pub_id = \''.(int)$pub_id.'\' AND c_user NOT IN (\''.$tsUser->uid.'\', \''.(int)$p_user.'\')'); 
        $data = result_array($query);
	...
	public function getNotificaciones($unread = false){
		...
		// SI VA AL MONITOR ENTONCES ACTUALIZAMOS PARA QUE YA NO SE VEAN EN EL MENUBAR
            ...
            $sql = 'SELECT m.*, u.user_name AS usuario FROM u_monitor AS m LEFT JOIN u_miembros AS u ON m.obj_user = u.user_id WHERE m.user_id = \''.$tsUser->uid.'\' ORDER BY m.not_id DESC';
               ...
               //ESTADÍSTICAS
              $dataDos['stats']['posts'] = mysql_num_rows(mysql_query('SELECT follow_id FROM u_follows WHERE f_user = \''.$tsUser->uid.'\' && f_type = \'3\''));
		      $dataDos['stats']['seguidores'] = mysql_num_rows(mysql_query('SELECT follow_id FROM u_follows WHERE f_id = \''.$tsUser->uid.'\' && f_type = \'1\''));
              $dataDos['stats']['siguiendo'] = mysql_num_rows(mysql_query('SELECT follow_id FROM u_follows WHERE f_user = \''.$tsUser->uid.'\' && f_type = \'1\''));
        ...
        // ACTUALIZAMOS
        if($this->show_type == 1) mysql_query('UPDATE u_monitor SET not_menubar = \''.(int)$not_del.'\' WHERE user_id = \''.$tsUser->uid.'\' AND not_menubar > 0');
        else mysql_query('UPDATE u_monitor SET not_menubar = \'0\', not_monitor = \'0\' WHERE user_id = \''.$tsUser->uid.'\' AND not_monitor = \'1\'');
	...
	function makeConsulta($data){
		...
		switch($data['not_type']){
            ...
            case 13:
                ...
                $query = mysql_query('SELECT p.pub_id, p.p_user, p.p_user_pub, u.user_name FROM u_muro AS p LEFT JOIN u_miembros AS u ON p.p_user = u.user_id WHERE p.pub_id = \''.(int)$data['obj_uno'].'\' LIMIT 1');
				$dato = mysql_fetch_assoc($query);
	...
	*/
	public function setFollow(){
		...
		// YA EXISTE?
		$query = mysql_query('SELECT follow_id FROM u_follows WHERE f_user = \''.$tsUser->uid.'\' AND f_id = \''.(int)$fw['obj'].'\' AND f_type = \''.$fw['type'].'\' LIMIT 1');
		$data = mysql_fetch_assoc($query);
		
		// SEGUIR
		if(empty($data['follow_id'])){
            if($tsUser->uid == $fw['obj'] && $fw['type'] == 1) return '1-'.$fw['obj'].'-0-No puedes seguirte a ti mismo.';
			if(mysql_query('INSERT INTO `u_follows` (`f_user`, `f_id`, `f_type`, `f_date`) VALUES (\''.$tsUser->uid.'\', \''.(int)$fw['obj'].'\', \''.$fw['type'].'\', \''.time().'\')')){
				// MONITOR?
				if($fw['notUser'] > 0) $this->setNotificacion($notType, $fw['notUser'], $tsUser->uid);
				// CUANTOS?
				$query = mysql_query('SELECT COUNT(follow_id) AS total FROM u_follows WHERE f_id = \''.(int)$fw['obj'].'\' AND f_type = \''.(int)$fw['type'].'\'');
	...
	*/
	public function setUnFollow(){
		...
		// DEJAR DE SEGUIR
		if(mysql_query('DELETE FROM u_follows WHERE f_user = \''.$tsUser->uid.'\' AND f_id = \''.(int)$fw['obj'].'\' AND f_type = \''.$fw['type'].'\'')){
				// CUANTOS?
				$total= mysql_num_rows(mysql_query('SELECT follow_id FROM u_follows WHERE f_id = \''.(int)$fw['obj'].'\' AND f_type = \''.$fw['type'].'\''));
				// RESPUESTA
	...
	public function getFollows($type, $user_id = 0){
		...
		switch($type){
			case 'seguidores':
                ...
                $data['pages'] = $pages;
                //
				$dato = result_array(mysql_query($query.' LIMIT '.$pages['limit']));
				
				//
				foreach($dato as $key => $val){
					$query = mysql_query('SELECT follow_id FROM u_follows WHERE f_user = \''.(int)$user_id.'\' AND f_id = \''.(int)$val['user_id'].'\' AND f_type = \'1\'');
					$siguiendo = mysql_fetch_assoc($query);
	...
	*/
	public function setSpam(){
		...
		$postid = $_POST['postid'];
        // TIENE SEGUIDORES?
		$query = mysql_query('SELECT follow_id FROM u_follows WHERE f_id = \''.$tsUser->uid.'\'  AND f_type = \'1\' LIMIT 1');
        $seguidores = mysql_num_rows($query);
        // YA LO HA RECOMENDADO?
		$query = mysql_query('SELECT follow_id FROM u_follows WHERE f_id = \''.(int)$postid.'\' AND f_user = \''.$tsUser->uid.'\' AND f_type = \'3\' LIMIT 1');
		...
		//
		$query = mysql_query('SELECT post_user FROM p_posts WHERE post_id = \''.(int)$postid.'\' LIMIT 1');
		$data = mysql_fetch_assoc($query);
		
		//
		if($tsUser->uid != $data['post_user']){
            // GUARDAMOS EN FOLLOWS PUES ES LA RECOMENDACION PARA SU SEGUIDORES! xD
			mysql_query('INSERT INTO u_follows (f_id, f_user, f_type, f_date) VALUES (\''.(int)$postid.'\', \''.$tsUser->uid.'\', \'3\', \''.time().'\') ');
			// NOTIFICAR
