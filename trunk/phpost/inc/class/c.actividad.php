...
class tsActividad {
    ...
    */
    ...
    public function setActividad($ac_type, $obj_uno, $obj_dos = 0){
		...
		$query = mysql_query('SELECT `ac_id` FROM `u_actividad` WHERE user_id = \''.$tsUser->uid.'\' ORDER BY ac_date DESC');
		...
		mysql_query('DELETE FROM `u_actividad` WHERE `ac_id` = '.$delid);
		}
        # SE HACE UN CONTEO PROGRESIVO SI HACE ESTA ACCON MAS DE 1 VEZ AL DIA
        if($ac_type == 5) {
            //
            $query = mysql_query('SELECT `ac_id, ac_date` FROM `u_actividad` WHERE user_id = '.$tsUser->uid.' AND obj_uno = '.$obj_uno.' AND ac_type = '.$ac_type.' LIMIT 1');
        ...
        # INSERCION DE DATOS        
		if(mysql_query('INSERT INTO `u_actividad` (`user_id`, `obj_uno`, `obj_dos`, `ac_type`, `ac_date`) VALUES ('.$tsUser->uid.', '.$obj_uno.', '.$obj_dos.', '.$ac_type.', '.$ac_date.')')) return true;
        		else return false;
    }
    /**
     * @name getActividad
