...
class tsMuro {
    ...
    public function streamPost(){
        ...
        switch($type){
            ...
            case 'status':
                ...
                $tsCore->antiFlood();
                //				//
				if(mysql_query('INSERT INTO u_muro (p_user, p_user_pub, p_body, p_date, p_type, p_ip) VALUES (\''.(int)$pid.'\', \''.$tsUser->uid.'\', \''.$data.'\', \''.$date.'\', \'1\', \''.$_SERVER['REMOTE_ADDR'].'\') ')){
            ...
            case 'foto':
                ...
                $tsCore->antiFlood();
                // INSERTAMOS
				if(mysql_query('INSERT INTO u_muro (p_user, p_user_pub, p_body, p_date, p_type, p_ip) VALUES (\''.(int)$pid.'\', \''.$tsUser->uid.'\', \''.$tsCore->setSecure($data. true).'\', \''.$date.'\', \'2\', \''.$tsCore->setSecure($_SERVER['REMOTE_ADDR']).'\')')){
                    $pub_id = mysql_insert_id();
                    // INSERTAR ADJUNTO
					if(mysql_query('INSERT INTO u_muro_adjuntos (pub_id, a_url, a_img) VALUES (\''.(int)$pub_id.'\', \''.$tsCore->setSecure($foto, true).'\', \''.$tsCore->setSecure($foto, true).'\') ')){
                        $type = 'mfoto';
                        // RETORNAMOS DATOS PARA EL TEMPLATE
            ...
            case 'enlace':
                ...
                $tsCore->antiFlood();
                // INSERTAR
				if(mysql_query('INSERT INTO u_muro (p_user, p_user_pub, p_body, p_date, p_type, p_ip) VALUES (\''.(int)$pid.'\', \''.$tsUser->uid.'\', \''.$data.'\', \''.$date.'\', \'3\', \''.$_SERVER['REMOTE_ADDR'].'\' ) ')){
                    $pub_id = mysql_insert_id();
                    // INSERTAR ADJUNTO
                    if(mysql_query('INSERT INTO u_muro_adjuntos (pub_id, a_title, a_url) VALUES (\''.(int)$pub_id.'\', \''.$tsCore->setSecure($tsCore->parseBadWords($enlace['title']), true).'\', \''.$tsCore->setSecure($tsCore->parseBadWords($enlace['url']), true).'\') ')){
                        $type = 'mlink';
                        // RETORNAMOS DATOS PARA EL TEMPLATE
            ...
            case 'video':
                ...
                $tsCore->antiFlood();
                // INSERTAR
				if(mysql_query('INSERT INTO u_muro (p_user, p_user_pub, p_body, p_date, p_type, p_ip) VALUES (\''.(int)$pid.'\', \''.$tsUser->uid.'\', \''.$tsCore->setSecure($data. true).'\', \''.$date.'\', \'4\', \''.$tsCore->setSecure($_SERVER['REMOTE_ADDR']).'\') ')){
                    $pub_id = mysql_insert_id();
                    // INSERTAR ADJUNTO
					if(mysql_query('INSERT INTO u_muro_adjuntos (pub_id, a_title, a_url, a_img, a_desc) VALUES (\''.(int)$pub_id.'\', \''.$tsCore->setSecure($tsCore->parseBadWords($video['title']), true).'\', \''.$video['ID'].'\', \'\', \''.$tsCore->setSecure($tsCore->parseBadWords($video['desc']), true).'\') ')){
                        $type = 'mvideo';
                        // RETORNAMOS DATOS PARA EL TEMPLATE
    ...
    */
    function streamRepost(){
        ...
        $pid = intval($_POST['pid']);
        //
       $query = mysql_query('SELECT `p_user`, `p_user_pub` FROM `u_muro` WHERE `pub_id` = \''.(int)$pid.'\' LIMIT 1');
       $pub = mysql_fetch_assoc($query);
        ...
        if($pub['p_user'] > 0){
            ...
            $date = time();
			$_SERVER['REMOTE_ADDR'] = $_SERVER['X_FORWARDED_FOR'] ? $_SERVER['X_FORWARDED_FOR'] : $_SERVER['REMOTE_ADDR'];
            if(mysql_query('INSERT INTO `u_muro_comentarios` (`pub_id`, `c_user`, `c_date`, `c_body`, `c_ip`) VALUES (\''.(int)$pid.'\', \''.$tsUser->uid.'\', \''.$date.'\', \''.$tsCore->setSecure($data, true).'\', \''.$_SERVER['REMOTE_ADDR'].'\')')){
                $cid = mysql_insert_id();
                // MONITOR
                ...
                // UPDATES
                mysql_query('UPDATE `u_muro` SET `p_comments` = p_comments + 1 WHERE `pub_id` = \''.(int)$pid.'\'');
                // PARA LA PANTILLA
    ...
    */
    function getNews($start = 0, $limit = 10){
        ...
        while($row = mysql_fetch_array($query)){
            ...
            // CARGAR ADJUNTOS
            if($row['p_type'] != 1){
                $queryDos = mysql_query('SELECT * FROM u_muro_adjuntos WHERE pub_id = \''.$row['pub_id'].'\' LIMIT 1');
                $adj = mysql_fetch_assoc($queryDos);
    ...
    */
    function getWall($user_id, $start = 0){
        global $tsCore;
        // PUBLICACION
        $query = mysql_query('SELECT p.*, u.user_name FROM u_muro AS p LEFT JOIN u_miembros AS u ON p.p_user_pub = u.user_id WHERE p.p_user = \''.(int)$user_id.'\' ORDER BY p.pub_id DESC LIMIT '.$start.',10');
        while($row = mysql_fetch_array($query)){
            // CARGAR LIKES
            if($row['p_likes'] > 0){
                $row['likes'] = $this->getPubExtras($row['pub_id'], 'likes', $row['p_likes']);
    ...
    function getPubExtras($pub_id, $type = 'likes', $likes = 0){
        ...
        switch($type){
            case 'likes':
                ...
                // VEMOS SI ME GUSTA
                if($tsUser->is_member){
                    $query = mysql_query('SELECT `like_id` FROM `u_muro_likes` WHERE `user_id` = \''.$tsUser->uid.'\' AND `obj_id` = \''.(int)$pub_id.'\' AND obj_type = \'1\'');
                    $i_like = mysql_num_rows($query);
                ...
                if($likes == 1){
                    if($i_like) {
                        ...
                        $query = mysql_query('SELECT u.user_name FROM u_muro_likes AS l LEFT JOIN u_miembros AS u ON l.user_id = u.user_id  WHERE l.obj_id = \''.(int)$pub_id.'\' AND l.obj_type = \'1\'');
                        $u_like = mysql_fetch_assoc($query);
                    ...
                    if($i_like){
                        $data['link'] = 'Ya no me gusta';
                        //
                        $query = mysql_query('SELECT u.user_name FROM u_muro_likes AS l LEFT JOIN u_miembros AS u ON l.user_id = u.user_id  WHERE l.user_id != \''.$tsUser->uid.'\' AND l.obj_id = \''.(int)$pub_id.'\' AND l.obj_type = 1');
                        $u_like = mysql_fetch_assoc($query);
            ...
            case 'comments':
                ...
                //
                $query = mysql_query('SELECT c.*, u.user_name FROM u_muro_comentarios AS c LEFT JOIN u_miembros AS u ON c.c_user = u.user_id WHERE c.pub_id = \''.(int)$pub_id.'\' ORDER BY c.c_date DESC '.$limit.'');
                while($row = mysql_fetch_array($query)){
                    ...
                    if($row['c_likes'] > 0){
                        //
                        $cuery = mysql_query('SELECT `like_id` FROM `u_muro_likes` WHERE `user_id` = \''.$tsUser->uid.'\' AND `obj_id` = \''.$row['cid'].'\' AND `obj_type` = \'2\'');
                        $i_like = mysql_num_rows($cuery);
    ...
    */
    function getStory($pub_id, $user_id){
        global $tsUser;
        // ELEGIMOS
        $query = mysql_query('SELECT p.*, u.user_name FROM u_muro AS p LEFT JOIN u_miembros AS u ON p.p_user_pub = u.user_id WHERE p.pub_id = \''.(int)$pub_id.'\' LIMIT 1');
        $pub = mysql_fetch_assoc($query);
        ...
        // ADJUNTOS
        if($pub['p_type'] != 1){
            $query = mysql_query('SELECT * FROM u_muro_adjuntos WHERE pub_id = \''.(int)$pub_id.'\' LIMIT 1');
            $adj = mysql_fetch_assoc($query);
    ...
    */
    function getComments(){
        ...
        $pid = (int) $_POST['pid'];
        // EXISTE?
        $query = mysql_query('SELECT `p_user`, `p_comments` FROM `u_muro` WHERE `pub_id` = \''.$pid.'\' LIMIT 1');
        $cmts = mysql_fetch_assoc($query);
    ...
    */
    function deletePost(){
        ...
        switch($type){
            case 'pub':
                // DATOS -robert
                $query = mysql_query('SELECT `p_user`, `p_user_pub` FROM `u_muro` WHERE `pub_id` = \''.(int)$id.'\' LIMIT 1');
                $data = mysql_fetch_assoc($query);
                ...
                if(!empty($data['p_user'])){
                    ...
                    if($data['p_user'] == $tsUser->uid || $data['p_user_pub'] == $tsUser->uid || $tsUser->is_admod || $tsUser->permisos['moepm']){
                       if(mysql_query('DELETE FROM `u_muro` WHERE `pub_id` = \''.(int)$id.'\'')){
                            // BORRAMOS LOS LIKES DE LA PUBLICACION
                           mysql_query('DELETE FROM `u_muro_likes` WHERE `obj_id` = \''.(int)$id.'\' AND `obj_type` = \'1\'');
                            // BORRAMOS LOS LIKES DE TODOS LOS COMENTARIOS
                           $query = mysql_query('SELECT `cid` FROM `u_muro_comentarios` WHERE `pub_id` = \''.(int)$id.'\'');
                            ...
                            mysql_query('DELETE FROM `u_muro_likes` WHERE `obj_id` IN (\''.$delete_ids.'\') AND `obj_type` = \'2\'');
                            // BORRAR COMENTARIOS
                            mysql_query('DELETE FROM `u_muro_comentarios` WHERE `pub_id` = \''.(int)$id.'\'');
                            mysql_query('DELETE FROM `u_muro_adjuntos` WHERE `pub_id` = \''.(int)$id.'\'');
                            //
                            return '1: OK';
                        } else return '0: Error';
            ...
            case 'cmt':
                // DATOS
                $query = mysql_query('SELECT c.cid, c.c_user, p.pub_id, p.p_user FROM u_muro_comentarios AS c LEFT JOIN u_muro AS p ON c.pub_id = p.pub_id WHERE c.cid = \''.(int)$id.'\' LIMIT 1');
                $data = mysql_fetch_assoc($query);
                ...
                if(!empty($data['cid'])){
                    ...
                    if($data['p_user'] == $tsUser->uid || $data['c_user'] == $tsUser->uid  || $tsUser->is_admod || $tsUser->permisos['moecm']){
                        if(mysql_query('DELETE FROM `u_muro_comentarios` WHERE `cid` = \''.(int)$id.'\'')){
                            // UPDATES
                            mysql_query('DELETE FROM `u_muro_likes` WHERE `obj_id` = \''.(int)$id.'\' AND `obj_type` = \'2\'');
                            mysql_query('UPDATE `u_muro` SET `p_comments` = p_comments - 1 WHERE `pub_id` = \''.$data['pub_id'].'\'');
                            //
                            return '1: Ok';
                        }
                    } else return '0: Hmmm... &iquest;Haciendo pruebas?';
   ...
   function likePost(){
        ...
        $exists = mysql_fetch_assoc($query);
        
        
        if(empty($exists['uid'])) return '0: La publicaci&oacute;n ya no existe.';
        // CHECAMOS SI YA LE GUSTA ESTO
		$query = mysql_query('SELECT like_id, user_id FROM u_muro_likes WHERE obj_id = \''.(int)$id.'\' AND obj_type = \''.(int)$type.'\'');
        ...
        // SI AUN NO ME GUSTA
        if(empty($i_like)){
			if(mysql_query('INSERT INTO u_muro_likes (user_id, obj_id, obj_type) VALUES (\''.$tsUser->uid.'\', \''.(int)$id.'\', \''.(int)$type.'\')')){
                // SUMAR LIKE
                if($type == 1) {
					mysql_query('UPDATE u_muro SET p_likes = p_likes + 1 WHERE pub_id = \''.(int)$id.'\'');
                    $ac_type = ($exists['uid'] == $tsUser->uid) ? 0 : 2; 
                }
                else {
					mysql_query('UPDATE u_muro_comentarios SET c_likes = c_likes + 1 WHERE cid = \''.(int)$id.'\'');
			...
			if(mysql_query('DELETE FROM u_muro_likes WHERE like_id = \''.(int)$i_like.'\'')){
                // RESTAR LIKE
				if($type == 1) mysql_query('UPDATE u_muro SET p_likes = p_likes - 1 WHERE pub_id = \''.(int)$id.'\'');
                else mysql_query('UPDATE u_muro_comentarios SET c_likes = c_likes - 1 WHERE cid = \''.(int)$id.'\'');
            }
            else $status = 'error';
        }
        // RESPUESTA
    ...
    */
    function showLikes(){
        ...
        $type = ($_POST['type'] == 'com') ? 2 : 1;
        //
        $query = mysql_query('SELECT l.user_id, u.user_name FROM u_muro_likes AS l LEFT JOIN u_miembros AS u ON l.user_id = u.user_id WHERE obj_id = \''.(int)$id.'\' AND obj_type = \''.(int)$type.'\'');
