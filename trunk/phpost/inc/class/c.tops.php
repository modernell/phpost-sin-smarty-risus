...
class tsTops {
    ...
    */
    function getTopUsers($fecha, $cat){
		...
		// PUNTOS
        $query = mysql_query('SELECT SUM(p.post_puntos) AS total, u.user_id, u.user_name FROM p_posts AS p LEFT JOIN u_miembros AS u ON p.post_user = u.user_id WHERE p.post_status = 0  AND p.post_date BETWEEN '.$data['start'].' AND '.$data['end'].' '.$category.' GROUP BY p.post_user ORDER BY total DESC LIMIT 10');
        $array['puntos'] = result_array($query);
        
        // SEGUIDORES
        $query = mysql_query('SELECT COUNT(f.follow_id) AS total, u.user_id, u.user_name FROM u_follows AS f LEFT JOIN u_miembros AS u ON f.f_id = u.user_id WHERE f.f_type = 1 AND f.f_date BETWEEN '.$data['start'].' AND '.$data['end'].' GROUP BY f.f_id ORDER BY total DESC LIMIT 10');
        $array['seguidores'] = result_array($query);
        
		// MEDALLAS
        $query = mysql_query('SELECT COUNT(m.medal_for) AS total, u.user_id, u.user_name, wm.medal_id FROM w_medallas_assign AS m LEFT JOIN u_miembros AS u ON m.medal_for = u.user_id LEFT JOIN w_medallas AS wm ON wm.medal_id = m.medal_id WHERE wm.m_type = \'1\' AND m.medal_date BETWEEN '.$data['start'].' AND '.$data['end'].' GROUP BY m.medal_for ORDER BY total DESC LIMIT 10');
	...
	*/
	function getTopPostsQuery($data){
		...
		//
        $query = mysql_query('SELECT p.post_id, p.post_category, '.$data['type'].', p.post_puntos, p.post_title, c.c_seo, c.c_img FROM p_posts AS p LEFT JOIN p_categorias AS c ON c.cid = p.post_category  WHERE p.post_status = \'0\' AND p.post_date BETWEEN '.$data['start'].' AND '.$data['end'].' '.$data['scat'].' ORDER BY '.$data['type'].' DESC LIMIT 10');
	...
	*/
	function getHomeTopPostsQuery($date){
		...
		//
		$query = mysql_query('SELECT p.post_id, p.post_category, p.post_title, p.post_puntos, c.c_seo FROM p_posts AS p LEFT JOIN p_categorias AS c ON c.cid = p.post_category  WHERE p.post_status = 0 AND p.post_date BETWEEN \''.$date['start'].'\' AND \''.$date['end'].'\' ORDER BY p.post_puntos DESC LIMIT 15');
    ...
    */
    function getHomeTopUsersQuery($date){
		// PUNTOS
        $query = mysql_query('SELECT SUM(p.post_puntos) AS total, u.user_id, u.user_name FROM p_posts AS p LEFT JOIN u_miembros AS u ON p.post_user = u.user_id WHERE p.post_status = 0  AND p.post_date BETWEEN \''.$date['start'].'\' AND \''.$date['end'].'\' GROUP BY p.post_user ORDER BY total DESC LIMIT 10');
        $data = result_array($query);
	...
	*/
	function getStats(){
        ...
        $return = mysql_fetch_assoc(mysql_query('SELECT stats_max_online, stats_max_time, stats_time, stats_time_cache, stats_miembros, stats_posts, stats_fotos, stats_comments, stats_foto_comments FROM w_stats WHERE stats_no = \'1\''));
        
        if($return['stats_time_cache'] < time()-($tsCore->settings['c_stats_cache']*60)){
        $q1 = mysql_fetch_row(mysql_query('SELECT COUNT(user_id) AS u FROM u_miembros WHERE user_activo = \'1\' && user_baneado = \'0\''));
		$q2 = mysql_fetch_row(mysql_query('SELECT COUNT(post_id) AS p FROM p_posts WHERE post_status = \'0\''));
        $q3 = mysql_fetch_row(mysql_query('SELECT COUNT(foto_id) AS f FROM f_fotos WHERE f_status = \'0\''));
        $q4 = mysql_fetch_row(mysql_query('SELECT COUNT(cid) AS c FROM p_comentarios WHERE c_status = \'0\''));
        $q5 = mysql_fetch_row(mysql_query('SELECT COUNT(cid) AS fc FROM f_comentarios'));
		...
		$cueri = mysql_fetch_row(mysql_query('SELECT COUNT(user_id) AS u FROM `u_miembros` WHERE `user_lastactive` > \''.$is_online.'\''));
		}else{
        ...
        mysql_query('UPDATE w_stats SET stats_time = \''.time().'\' '.$ndat.' '.$timen);
		//
		return $return;
	}
	/******************************************************************************/
