...
class tsRegistro{
	...
	global $tsCore;
		...
		if(!empty($username) || !empty($email))
                {
			$query = mysql_query('SELECT `user_id` FROM `u_miembros` WHERE LOWER(user_name) = \''.$tsCore->setSecure($username).'\' OR LOWER(user_email) = \''.$tsCore->setSecure($email).'\' LIMIT 1');
			if(($query) && (mysql_num_rows($query) > 0))
                        {    
                            $valid = '0: El '.$which.' ya se encuentra registrado.';	// EXISTE
                        }    
                        $query=mysql_query("SELECT id FROM w_blacklist WHERE (type = '3' && value = '".$tsCore->setSecure(strstr($email, '@'))."') ||  (type = \'4\' && value = \'".$tsCore->setSecure(strstr($email, '@', true))."\') || (type = \'4\' && value = \'".$tsCore->setSecure($username)."\') LIMIT 1");
	...
	function registerUser(){
		...
		//HACEMOS COMPROBACIONES DE DATOS DE NUEVO
		$query = mysql_query('SELECT `user_name`,`user_email` FROM `u_miembros` WHERE LOWER(user_name) = \''.$tsCore->setSecure($tsData['user_nick']).'\' OR LOWER(user_email) = \''.$tsCore->setSecure($tsData['user_email']).'\' LIMIT 1');
		if(mysql_num_rows($query) > 0 || !filter_var($tsData['user_email'], FILTER_VALIDATE_EMAIL) || $tsCore->settings['c_reg_active'] == 0) die('0: El Hubo problemas al intentar registrarle, hay campos vac&iacute;os, inv&aacute;lidos o no se le permite el registro.');
		
		// PASAMOS BIEN... AHORA INSERTAR DATOS
		$key = md5(md5($tsData['user_password']).strtolower($tsData['user_nick']));
		//
		if(mysql_query('INSERT INTO `u_miembros` (`user_name`, `user_password`, `user_email`, `user_rango`, `user_registro`) VALUES (\''.$tsCore->setSecure($tsData['user_nick']).'\', \''.$tsCore->setSecure($key).'\', \''.$tsCore->setSecure($tsData['user_email']).'\', '.(empty($tsCore->settings['c_reg_rango']) ? 3 : $tsCore->settings['c_reg_rango']).', \''.$tsData['user_registro'].'\')')){
            $tsData['user_id'] = mysql_insert_id();
            // INSERTAMOS EL PERFIL
			mysql_query('INSERT INTO `u_perfil` (`user_id`, `user_dia`, `user_mes`, `user_ano`, `user_pais`, `user_estado`, `user_sexo`) VALUES (\''.(int)$tsData['user_id'].'\', \''.(int)$tsData['user_dia'].'\', \''.(int)$tsData['user_mes'].'\', \''.(int)$tsData['user_anio'].'\', \''.$tsData['user_pais'].'\', \''.$tsData['user_estado'].'\', \''.(int)$tsData['user_sexo'].'\')');
            mysql_query('INSERT INTO `u_portal` (`user_id`) VALUES (\''.(int)$tsData['user_id'].'\')');
			...
			if($tsData['user_sexo'] == 1)  $sexo = 'Bienvenido'; else $sexo = 'Bienvenida'; 
			...
			switch($send_welcome) {
            case 1:
			mysql_query('INSERT INTO `u_muro` (p_user, p_user_pub, p_date, p_body, p_type) VALUES (\''.(int)$tsData['user_id'].'\', \'1\', \''.time().'\', \''.$msg_bienvenida.'\', \'1\')'); 
            $m_id = mysql_insert_id();
			mysql_query('INSERT INTO `u_monitor` (user_id,obj_user,obj_uno, not_type,not_total,not_menubar,not_monitor) VALUES (\''.(int)$tsData['user_id'].'\', \'1\', \''.$m_id.'\', \'12\', \'1\', \'1\', \'1\')');
			break;
            case 2:
			$preview = substr($msg_bienvenida,0,75); 
			if(mysql_query('INSERT INTO `u_mensajes` (`mp_to`, `mp_from`, `mp_subject`, `mp_preview`, `mp_date`) VALUES ('.$tsData['user_id'].', \'1\', \''.$sexo.' a ' .$tsCore->settings['titulo'].'\', \''.$preview.'\',\''.time().'\')')) {
            $mp_id = mysql_insert_id();
            mysql_query('INSERT INTO `u_respuestas` (mp_id, mr_from, mr_body, mr_ip, mr_date) VALUES (\''.(int)$mp_id.'\', \'1\', \''.$msg_bienvenida.'\',  \''.$_SERVER['REMOTE_ADDR'].'\', \''.time().'\')'); }
			break;
		    case 3:
			mysql_query('INSERT INTO `u_avisos` (`user_id`, `av_subject`, `av_body`, `av_date`, `av_type`) VALUES (\''.$tsData['user_id'].'\', \''.$sexo.' a ' .$tsCore->settings['titulo'].'\', \''.$msg_bienvenida.'\', \''.time().'\', \'4\')');			
            break;
			}
		}
			// ENVIAMOS EL EMAIL
			...
			if(mysql_query('INSERT INTO w_contacts (user_id, user_email, time, type, hash) VALUES (\''.(int)$tsData['user_id'].'\', \''.$tsCore->setSecure($tsData['user_email']).'\', \''.time().'\', \'2\', \''.$key.'\' )')){	
