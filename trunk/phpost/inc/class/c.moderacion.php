...
class tsMod
{
    ...
    */
    function getMods()
    {
        ...
        //
        $query = mysql_query('SELECT `user_id`, `user_name` FROM `u_miembros` WHERE `user_rango` = \'2\' ORDER BY `user_id`');
        //
    ...
    function getDenuncias($type = 'posts')
    {
        ...
        switch ($type)
        {
            case 'posts':
                ...
                break;
            case 'fotos':
                $query = mysql_query('SELECT r.*, SUM(d_total) AS total, f.foto_id, f.f_title, f.f_status, u.user_id, u.user_name FROM w_denuncias AS r LEFT JOIN f_fotos AS f ON r.obj_id = f.foto_id LEFT JOIN u_miembros AS u ON f.f_user = u.user_id  WHERE d_type = \'4\' && f.f_status < 2 GROUP BY r.obj_id ORDER BY total DESC, r.d_date DESC');
                $data = result_array($query);
                
                break;
            case 'users':
                $query = mysql_query('SELECT r.*, SUM(d_total) AS total, u.user_name FROM w_denuncias AS r LEFT JOIN u_miembros AS u ON r.obj_id = u.user_id WHERE d_type = 3 AND u.user_baneado = 0 GROUP BY r.obj_id ORDER BY total, r.d_date DESC');
                $data = result_array($query);
    ...
    */
    function getDenuncia($type = 'posts')
    {
        ...
        switch ($type)
        {
            case 'posts':
                $d_type = 1;
                $query = mysql_query('SELECT p.post_id, p.post_title, p.post_status, c.c_nombre, c.c_seo, c.c_img, u.user_name FROM p_posts AS p LEFT JOIN p_categorias AS c ON p.post_category = c.cid LEFT JOIN u_miembros AS u ON p.post_user = u.user_id WHERE p.post_id = ' .
                    $obj . ' LIMIT 1');
                break;
            case 'fotos':
                $d_type = 4;
                $query = mysql_query('SELECT f.foto_id, f.f_title, f.f_status, u.user_name FROM f_fotos AS f LEFT JOIN u_miembros AS u ON f.f_user = u.user_id WHERE f.foto_id = ' .
                    $obj . ' LIMIT 1');
                break;
            case 'users':
                $d_type = 3;
        ...
        // CARGAMOS AL ARRAY...
        $data['data'] = mysql_fetch_assoc($query);
        
        // DENUNCIAS
        $query = mysql_query("SELECT d.*, u.user_id, u.user_name FROM w_denuncias AS d LEFT JOIN u_miembros AS u ON d.d_user = u.user_id WHERE d.obj_id = {$obj} AND d.d_type = {$d_type}");
    ...
    function getContenido()
    {
        ...
        if ($metodo == 1)
            ...
            $met = ' = \'' . $texto . '\'';
        //
        $query = mysql_query('SELECT m.pub_id, m.p_user, m.p_user_pub, m.p_ip, m.p_date, m.p_body, u.user_id, u.user_name FROM u_muro AS m LEFT JOIN u_miembros AS u ON m.p_user_pub = u.user_id WHERE ' .
            ($tipo == 1 ? 'm.p_ip' : 'm.p_body') . '  ' . $met);
        $data['muro'] = result_array($query);
        $data['m_total'] = count($data['muro']);
        
        //
        $query = mysql_query('SELECT user_id, user_name, user_last_ip, user_lastlogin, user_lastactive FROM u_miembros WHERE ' .
        ...
        //
        $query = mysql_query('SELECT p.post_id, p.post_user, p.post_title, p.post_date, p.post_ip, u.user_name, c.c_nombre, c.c_seo, c.c_img FROM p_posts AS p LEFT JOIN u_miembros AS u ON p.post_user = u.user_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE ' .
        ...
        //
        $query = mysql_query('SELECT u.user_id, u.user_name, c.* FROM p_comentarios AS c LEFT JOIN u_miembros AS u ON u.user_id = c.c_user WHERE ' .
    ...
    */
    ...
    {
        global $tsCore;
        $query = mysql_query('SELECT `post_title`, `post_body` FROM `p_posts` WHERE `post_id` = \'' .
            $pid . '\' LIMIT 1');
    ...
    public function rebootPost($pid)
    {
        ...
        if ($tsUser->is_admod || $tsUser->permisos['mocdp'])
        {
            ...
            $datos = mysql_fetch_assoc(mysql_query('SELECT post_id, post_status FROM p_posts WHERE post_id = \'' .
                (int)$pid . '\' LIMIT 1'));
            if ($datos['post_status'] == 3)
            {
                if (!mysql_query('DELETE FROM `w_historial` WHERE `pofid` = \'' . (int) $pid . '\' && `type` = \'1\' && `action` = \'3\''))
                    return '0: No se pudo restaurar el post.';
            } else
            {
                //BORRAMOS LA DENUNCIAS
                if (!mysql_query('DELETE FROM `w_denuncias` WHERE `obj_id` = \'' . (int) $pid . '\' AND `d_type` = \'1\''))
                    return '0: No se pudo restaurar el post.';
            }
            // REGRESAMOS EL POST
            if (mysql_query('UPDATE `p_posts` SET `post_status` = \'0\' WHERE `post_id` = \'' .
                $pid . '\''))
            {
    ...
    public function OcultarPost($pid, $razon)
    {
        ...
        {
            if (!mysql_num_rows(mysql_query('SELECT post_id FROM p_posts WHERE post_id = \'' .
                (int)$pid . '\' && post_status = \'3\'')))
            {
                if (mysql_query('UPDATE p_posts SET post_status = \'3\' WHERE post_id = \'' . (int)
                    $pid . '\''))
                {
                    if (mysql_query('INSERT INTO `w_historial` (`pofid`, `action`, `type`, `mod`, `reason`, `date`, `mod_ip`) VALUES (\'' .
    ...
    function rebootMps($mid)
    {
        ...
        if ($tsUser->is_admod || $tsUser->permisos['mocdm'])
        {
            ...
            if (mysql_query('DELETE FROM `w_denuncias` WHERE `obj_id` = \'' . (int)$mid . '\' AND `d_type` = \'2\''))
            {
                mysql_query('UPDATE `u_mensajes` SET mp_del_to = \'0\', mp_del_from = \'0\' WHERE `mp_id` = \'' .
                    (int)$mid . '\'');
    ...
    function rebootFoto($fid)
    {
        ...
        if ($tsUser->is_admod || $tsUser->permisos['mocdf'])
        {
            ...
            //BORRAMOS LA DENUNCIA
            if (mysql_query('DELETE FROM `w_denuncias` WHERE `obj_id` = \'' . (int)$fid . '\' AND `d_type` = \'4\''))
            {
                mysql_query('UPDATE `f_fotos` SET f_status= \'0\' WHERE `foto_id` = \'' . (int)
                    $fid . '\'');
                return '1: Denuncia eliminada';
            } else
                return '0: No se pudo eliminar la denuncia';
    ...
    public function deletePost($pid)
    {
        ...
        if ($tsUser->is_admod || $tsUser->permisos['moep'])
        {
            ...
            //
            if (mysql_query('UPDATE `p_posts` SET `post_status` = \'2\' WHERE `post_id` = \'' .
                $pid . '\''))
            {
                // ELIMINAR DENUNCIAS
                mysql_query('DELETE FROM `w_denuncias` WHERE `obj_id` = \'' . $pid . '\' AND `d_type` = \'1\'');
                // ENVIAR AVISO
                $query = mysql_query('SELECT p.post_user, p.post_title, p.post_body, p.post_tags, p.post_category, u.user_name, u.user_email FROM p_posts AS p LEFT JOIN u_miembros AS u ON p.post_user = u.user_id WHERE p.post_id = \'' .
                    (int)$pid . '\' LIMIT 1');
                $data = mysql_fetch_assoc($query);
                ...
                if ($_POST['send_b'] == 'yes')
                    mysql_query('INSERT INTO `p_borradores` (b_user, b_date, b_title, b_body, b_tags, b_category, b_status, b_causa) VALUES (\'' .
    ...
    public function deleteMps($mid)
    {
        ...
        {
            // ENVIAR AVISO
            if ($query = mysql_query('SELECT m.mp_from, m.mp_subject, u.user_name FROM u_mensajes AS m LEFT JOIN u_miembros AS u ON m.mp_from = u.user_id WHERE m.mp_id = \'' .
                (int)$mid . '\' LIMIT 1'))
            {
                $data = mysql_fetch_assoc($query);
                ...
                // ELIMINAR DENUNCIAS
                mysql_query('DELETE FROM `w_denuncias` WHERE `obj_id` = \'' . $mid . '\' AND `d_type` = \'2\'');
                //LOS MPS SE ELIMINARAN DE LA LISTA DE MPS DEL USUARIO, PERO NO SE BORRARÁN.
                mysql_query('UPDATE `u_mensajes` SET mp_del_to = \'1\', mp_del_from = \'1\' WHERE `mp_id` = \'' .
                    (int)$mid . '\'');
                ...
                //mysql_query('DELETE FROM `u_respuestas` WHERE `mp_id` = \''.$mid.'\'');
                //mysql_query('DELETE FROM `u_mensajes`   WHERE `mp_id` = \''.$mid.'\'');
                //
    ...
    public function deleteFoto($fid)
    {
        ...
        if ($tsUser->is_admod || $tsUser->permisos['moadf'] || $tsUser->permisos['moef'])
        {
            ...
            //
            if (mysql_query('UPDATE `f_fotos` SET `f_status` = \'2\' WHERE `foto_id` = \'' .
                $fid . '\''))
            {
                ...
                {
                    // ENVIAR AVISO
                    $query = mysql_query('SELECT f.f_user, f.f_title, u.user_name FROM f_fotos AS f LEFT JOIN u_miembros AS u ON f.f_user = u.user_id WHERE f.foto_id = \'' .
                        (int)$fid . '\' LIMIT 1');
                    $data = mysql_fetch_assoc($query);
                ...
                // ELIMINAR DENUNCIAS
                mysql_query('DELETE FROM `w_denuncias` WHERE `obj_id` = \'' . $fid . '\' AND `d_type` = \'4\'');
    ...
    public function setSticky($post_id)
    {
        ...
        {
            //
            $query = mysql_query('SELECT `post_sticky` FROM `p_posts` WHERE `post_id` = \'' .
               (int) $post_id . '\' LIMIT 1');
            $data = mysql_fetch_assoc($query);
            ...
            {
                mysql_query('UPDATE `p_posts` SET `post_sticky` = \'0\' WHERE `post_id` = \'' .
                   (int) $post_id . '\'');
                return '1: El post fue quitado de la home.';
            } else
            {
                mysql_query('UPDATE `p_posts` SET `post_sticky` = \'1\' WHERE `post_id` = \'' .
                    (int) $post_id . '\'');
    ...
    public function setOpenClosed($post_id)
    {
        ...
        {
            //
            $query = mysql_query('SELECT `post_block_comments` FROM `p_posts` WHERE `post_id` = \'' .
                (int) $post_id . '\' LIMIT 1');
            $data = mysql_fetch_assoc($query);
            ...
            {
                if (mysql_query('UPDATE `p_posts` SET `post_block_comments` = \'0\' WHERE `post_id` = \'' .
                    (int) $post_id . '\''))
            ...
            {
                if (mysql_query('UPDATE `p_posts` SET `post_block_comments` = \'1\' WHERE `post_id` = \'' .
                    (int) $post_id . '\''))
    ...
    public function getSuspendidos()
    {
        ...
        { // Para más seguridad, no se cargarán datos si no tiene permisos.
            ...
            $query = mysql_query('SELECT s.*, u.user_name FROM u_suspension AS s LEFT JOIN u_miembros AS u ON s.user_id = u.user_id WHERE 1 ORDER BY ' .
                $order . ' ' . $met . ' LIMIT ' . $limit);
            $data['bans'] = result_array($query);
            
            // PAGINAS
            $query = mysql_query('SELECT COUNT(*) FROM u_suspension WHERE user_id > \'0\'');
    ...
    public function banUser($user_id)
    {
        ...
        # COMPROBAMOS RANGOS
        $query = mysql_query('SELECT `user_rango`, `user_baneado` FROM `u_miembros` WHERE `user_id` = \'' .
            (int)$user_id . '\' LIMIT 1');
        $data = mysql_fetch_assoc($query);
        ...
        if ($data['user_baneado'] == 0)
        {
                ...
                permisos['mosu'] && $data['user_rango'] >= 2))
            {
                ...
                // ACTUALIZAMOS
                mysql_query('UPDATE `u_miembros` SET `user_baneado` = \'1\' WHERE `user_id` = \'' .
                    (int)$user_id . '\'');
                if (mysql_query('INSERT INTO `u_suspension` (`user_id`, `susp_causa`, `susp_date`, `susp_termina`, `susp_mod`, `susp_ip`) VALUES (\'' .
                    (int)$user_id . '\', \'' . $b_causa . '\', \'' . (int)
                    $ahora . '\',  \'' . (int)$termina . '\', \'' . $tsUser->uid . '\', \'' .
                    $tsCore->setSecure($_SERVER['REMOTE_ADDR']) . '\')'))
                {
                    // ELIMINAR DENUNCIAS
                    mysql_query('DELETE FROM `w_denuncias` WHERE `obj_id` = \'' . (int)$user_id . '\' AND `d_type` = \'3\'');
                    // RESTAR USUARIO EN ESTADÍSTICAS
                    mysql_query('UPDATE `w_stats` SET `stats_miembros` = stats_miembros - \'1\' WHERE `stats_no` = \'1\'');
                    // RETORNAR
                    if ($b_time < 2)
                    {
    ...
    public function rebootUser($user_id, $type = 'unban')
    {
        ...
        if ($tsUser->is_admod || $tsUser->permisos['modu'])
        {
            ...
            if ($type == 'unban')
            {
                $query = mysql_query('SELECT `susp_mod` FROM `u_suspension` WHERE `user_id` = \'' .
                    (int)$user_id . '\'');
                $data = mysql_fetch_assoc($query);
                ...
                {
                    mysql_query('DELETE FROM `u_suspension` WHERE `user_id` = \'' . (int)$user_id .
                        '\'');
                    mysql_query('UPDATE `u_miembros` SET `user_baneado` = \'0\' WHERE `user_id` = \'' .
                        (int)$user_id . '\'');
                    mysql_query('UPDATE `w_stats` SET `stats_miembros` = stats_miembros + \'1\' WHERE `stats_no` = \'1\'');
    ...
    public function setHistory($action, $type, $data)
    {
        ...
        if ($type == 'post')
        {
            switch ($action)
            {
                case 'borrar':
                    ...
                    $razon_db = ($razon != 13) ? $razon : $razon_desc;
                    // DATOS
                    $query = mysql_query('SELECT `post_id`, `post_body`, `post_title`, `post_user`, `post_category` FROM `p_posts` WHERE `post_id` = \'' .(int)$data . '\' LIMIT 1');
        ...
        {
            // DATOS
            $query = mysql_query('SELECT `foto_id`, `f_description`, `f_title`, `f_user` FROM `f_fotos` WHERE `foto_id` = \'' .
                (int)$data . '\' LIMIT 1');
            $foto = mysql_fetch_assoc($query);
    ...
    public function getPospelera()
    {
        ...
        // PAGINAS
        $query = mysql_query('SELECT COUNT(*) FROM p_posts AS p LEFT JOIN u_miembros AS u ON u.user_id = p.post_user LEFT JOIN w_historial AS h ON h.pofid = p.post_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category  WHERE h.type = \'1\' AND h.action = \'2\'');
        list($total) = mysql_fetch_row($query);
        ...
        $query = mysql_query('SELECT u.user_id, u.user_name, h.*, p.post_id, p.post_title, c.c_seo, c.c_nombre FROM p_posts AS p LEFT JOIN u_miembros AS u ON u.user_id = p.post_user LEFT JOIN w_historial AS h ON h.pofid = p.post_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category  WHERE h.type = \'1\' AND h.action = \'2\' AND p.post_status = \'2\' LIMIT ' .
            $limit);
        // DENUNCIAS
    ...
    public function getFopelera()
    {
        ...
        // PAGINAS
        $query = mysql_query('SELECT COUNT(*) FROM f_fotos AS f LEFT JOIN u_miembros AS u ON u.user_id = f.f_user LEFT JOIN w_historial AS h ON h.pofid = f.foto_id WHERE h.type = \'2\' AND h.action = \'2\' AND f.f_status = \'2\'');
        ...
        $query = mysql_query('SELECT u.user_id, u.user_name, h.*, f.foto_id, f.f_title, f.f_user FROM f_fotos AS f LEFT JOIN u_miembros AS u ON u.user_id = f.f_user LEFT JOIN w_historial AS h ON h.pofid = f.foto_id WHERE h.type = \'2\' AND h.action = \'2\' AND f.f_status = \'2\' LIMIT ' .
            $limit);
        // DENUNCIAS
        include ("../ext/datos.php");
        //
    ...
    public function getComentariosD()
    {
        ...
        // PAGINAS
        $query = mysql_query('SELECT COUNT(*) FROM p_comentarios AS c LEFT JOIN u_miembros AS u ON u.user_id = c.c_user WHERE c.c_status = \'1\'');
        ...
        $query = mysql_query('SELECT u.user_id, u.user_name, c.cid, c.c_user, c.c_post_id, c.c_date, c.c_body, c.c_ip, p.post_id, p.post_title, cat.c_seo, cat.c_nombre FROM p_comentarios AS c LEFT JOIN p_posts AS p ON c.c_post_id = p.post_id LEFT JOIN p_categorias AS cat ON cat.cid = p.post_category  LEFT JOIN u_miembros AS u ON u.user_id = c.c_user WHERE c.c_status = \'1\' ORDER BY c.c_date DESC LIMIT ' .
            $limit);
    ...
    public function getPostsD()
    {
        ...
        // PAGINAS
        $query = mysql_query('SELECT COUNT(*) FROM p_posts AS p LEFT JOIN u_miembros AS u ON u.user_id = p.post_user WHERE p.post_status = \'3\'');
        ...
        //
        $query = mysql_query('SELECT u.user_id, u.user_name, h.*, p.post_id, p.post_title, c.c_seo, c.c_nombre FROM p_posts AS p LEFT JOIN w_historial AS h ON h.pofid = p.post_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category LEFT JOIN u_miembros AS u ON u.user_id = h.mod  WHERE h.type = \'1\' AND h.action = \'3\' AND p.post_status = \'3\' LIMIT ' .
            $limit);
        $data['datos'] = result_array($query);
    ...
    public function getHistory($type)
    {
        ...
        //
        if ($type == 1)
            $query = mysql_query('SELECT u.user_id, u.user_name, h.*, p.post_id, p.post_title FROM p_posts AS p LEFT JOIN u_miembros AS u ON u.user_id = p.post_user LEFT JOIN w_historial AS h ON h.pofid = p.post_id WHERE h.type = \'1\' ORDER BY h.id DESC LIMIT 20');
        else
            $query = mysql_query('SELECT u.user_id, u.user_name, h.*, f.foto_id, f.f_title, f.f_user FROM f_fotos AS f LEFT JOIN u_miembros AS u ON u.user_id = f.f_user LEFT JOIN w_historial AS h ON h.pofid = f.foto_id WHERE h.type = \'2\' ORDER BY h.id DESC LIMIT 20');
        // DENUNCIAS
        include ("../ext/datos.php");
        //
        while ($row = mysql_fetch_assoc($query))
