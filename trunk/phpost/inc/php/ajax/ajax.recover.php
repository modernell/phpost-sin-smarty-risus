...
*/
	...
	$user_info = mysql_query('SELECT user_id, user_name, user_registro, user_activo FROM u_miembros WHERE user_email = \''.$email.'\'') or die('0: '.mysql_error());
	if(!mysql_num_rows($user_info)){
		die('0: <div class="dialog_box">El email no se encuentra registrado.</div>');
	}

	$tsData = mysql_fetch_assoc($user_info);
	...
	switch($action){
		case 'recover-pass':
			...
			mysql_query('INSERT INTO w_contacts (user_id, user_email, `time`, `type`, `hash`) VALUES (\''.$tsData['user_id'].'\', \''.$email.'\', \''.time().'\', \'1\', \''.$key.'\')');
		...
		if($tsData['user_activo'] == 1) die('0: La cuenta ya se encuentra activada');
			...
			$key = md5(sha1(uniqid(time().$email.microtime(true), true)));
			mysql_query('INSERT INTO w_contacts (user_id, user_email, `time`, `type`, `hash`) VALUES (\''.$tsData['user_id'].'\', \''.$email.'\', \''.time().'\', \'2\', \''.$key.'\')');
