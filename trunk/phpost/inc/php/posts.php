 ...
 */
    if(!empty($_GET['post_id'])){
    	...
    	if($tsPost['post_id'] > 0){
    		...
    		$smarty->assign("tsAutor",$tsPosts->getAutor($tsPost['post_user']));
                $tsAutor=$tsPosts->getAutor($tsPost['post_user']);
    ...
    else 
    {
    	...
    	$tsLastPosts = $tsPosts->getLastPosts($category, $subcateg, false);
    	$smarty->assign("tsPosts",$tsLastPosts['data']);
        $tsPosts=$tsLastPosts['data'];
        $smarty->assign("tsPages",$tsLastPosts['pages']);
        $tsPages=$tsLastPosts['pages'];
    	// ULTIMOS POSTS FIJOS        
        if($tsLastPosts['pages']['current'] == 1)
        {
    	   $newPosts= & tsPosts::getInstance();
            //$tsPosts=& tsPosts::getInstance();
           $tsLastStickys = $newPosts->getLastPosts($category, $subcateg, true);
    	   $smarty->assign("tsPostsStickys",$tsLastStickys['data']);
           $tsPostsStickys = $tsLastStickys['data'];           
        }
    	// CAT
    	$smarty->assign("tsCat",$category);
        $tsCat=$category;
    	$smarty->assign("tsStats",$tsTops->getStats());
        $tsStats=$tsTops->getStats();
    	// ULTIMOS COMENTARIOS
    	$smarty->assign("tsComments",$newPosts->getLastComentarios());
        $tsComments= $newPosts->getLastComentarios();
    	// TOP POSTS
    	$smarty->assign("tsTopPosts",$tsTops->getHomeTopPosts());
        $tsTopPosts=$tsTops->getHomeTopPosts();
    	// TOP USERS
    	$smarty->assign("tsTopUsers",$tsTops->getHomeTopUsers());
        $tsTopUsers=$tsTops->getHomeTopUsers();
        // TITULO
        if(!empty($category)) {
            $catData = $newPosts->getCatData();
