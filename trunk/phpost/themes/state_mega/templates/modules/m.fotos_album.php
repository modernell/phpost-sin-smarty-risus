                ...
                <div id="album" style="width: 100%;">
                	<div class="title-w clearfix">
                        <h2><? if ($tsFUser[0] == $tsUser->uid) echo 'Mis fotos'; else {echo 'Fotos de '; echo $tsFUser[1];} ?></h2>
                    </div>
                    <ul class="fotos-detail listado-content">
                    	...
                    	<li>
                            ...
                            <div class="notification-info">
                            	<span>
                                    <a href="<? echo $tsConfig['url'];?>/fotos/{$f.user_name}/{$f.foto_id}/{$f.f_title|seo}.html">{$f.f_title}</a><br /> 
                            		<span title="{$f.f_date|date_format:"%d.%m.%y a las %H:%M hs."}" class="time"><strong>{$f.f_date|date_format:"%d.%m.%Y"}</strong> - Por <a href="{$tsConfig.url}/perfil/{$f.user_name}">{$f.user_name}</a></span><hr />
                                        <span class="time"><? echo string_truncate($f['f_description'],100); ?></span>
                                </span>
                            </div>
                        </li>
                        {/foreach}
