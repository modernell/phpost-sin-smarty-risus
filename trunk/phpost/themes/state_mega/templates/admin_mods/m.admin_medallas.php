										...
										{foreach from=$tsMedals.medallas item=m}
											<tr id="medal_id_{$m.medal_id}">
												...
												<td>{if $m.m_autor == 0}Sistema{else}
                                                                                                    <a href="{$tsConfig.url}/perfil/{$m.user_name}" class="hovercard" uid="{$m.user_id}">{$m.user_name}</a>
                                                                                                    {/if}
                                                                                                </td>
												<td>{$m.m_date|date_format:"%d/%m/%Y"}</td>
												<td id="total_med_assig_{$m.medal_id}">{$m.m_total}</td>
												<td class="admin_actions">
												<a onclick="admin.medallas.asignar({$m.medal_id}); return false">
                                                                                                    <img src="{$tsConfig.default}/images/icons/plus.png" title="Asignar Medalla"/>
                                                                                                </a>
												<a href="{$tsConfig.url}/admin/medals/editar/{$m.medal_id}">
                                                                                                    <img src="{$tsConfig.default}/images/icons/editar.png" title="Editar Medalla"/>
                                                                                                </a>
												<a onclick="admin.medallas.borrar({$m.medal_id}); return false">
                                                                                                    <img src="{$tsConfig.default}/images/icons/close.png" title="Borrar Medalla" />
                                                                                                </a>
												</td>
											</tr>
										{/foreach}
									...
									<table cellpadding="0" cellspacing="0" border="0" class="admin_table" width="550" align="center">
										...
										{foreach from=$tsAsignaciones.asignaciones item=m}
											<tr id="assign_id_{$m.id}">
												...
												<td>{if $m.m_type == 1}Usuario{elseif $m.m_type == 2}Post{else}Foto{/if}</td>
												<td>{if $m.m_type == 1}
                                                                                                    <a href="{$tsConfig.url}/perfil/{$m.user_name}" class="hovercard" uid="{$m.user_id}">@{$m.user_name}</a>{elseif $m.m_type == 2}
                                                                                                    <a href="{$tsConfig.url}/posts/{$m.c_seo}/{$m.post_id}/{$m.post_title|seo}.html" target="_blank">{$m.post_title}</a>
                                                                                                    {else}
                                                                                                    <a href="{$tsConfig.url}/fotos/autor/{$m.foto_id}/{$m.f_title}.html" target="_blank">{$m.f_title}</a>{/if}
                                                                                                </td>
												<td>{$m.m_date|hace:true}</td>{*date_format:"%d/%m/%Y"*}
												<td>{$m.medal_ip}</td>
												<td class="admin_actions">
												<a onclick="admin.medallas.borrar_asignacion({$m.id}, {$m.medal_id}); return false">
                                                                                                    <img src="{$tsConfig.default}/images/icons/close.png" title="Borrar Asignaci&oacute;n" />
                                                                                                </a>
												</td>
											</tr>
										{/foreach}
									...
									<script type="text/javascript">
										...
										<dt><label for="rSpecial">Condici&oacute;n especial:</label><br />
											...
											<label onclick="$('#ai_cond_post').slideUp(); $('#ai_cond_foto').slideUp(); $('#ai_cond_user').slideDown(); $('#ai_cond_user_rango_span').slideDown();">
                                                                                            <input name="med_type" type="radio" id="ai_type" value="1" {if $tsMed.m_type == 1}checked{/if} class="radio"/>Usuario</label>
											<label onclick="$('#ai_cond_user').slideUp(); $('#ai_cond_user_rango').slideUp();  $('#ai_cond_foto').slideUp(); $('#ai_cond_post').slideDown();">
                                                                                            <input name="med_type" type="radio" id="ay_type" value="2" {if $tsMed.m_type == 2}checked{/if} class="radio"/>Post</label>
											<label onclick="$('#ai_cond_user').slideUp(); $('#ai_cond_user_rango').slideUp();  $('#ai_cond_post').slideUp(); $('#ai_cond_foto').slideDown();">
                                                                                            <input name="med_type" type="radio" id="ay_type" value="3" {if $tsMed.m_type == 3}checked{/if} class="radio"/>Foto</label>
										<span>consiga</span>
										</dt>
										<dd>						
