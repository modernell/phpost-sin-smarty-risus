                                        ...
                                        <tbody>{foreach from=$tsBlackList.data item=b}
                                        	<tr id="block_{$b.id}">
                                                ...
                                                <td>{$b.date|hace}</td>
                                                    ...
                                                    <a href="{$tsConfig.url}/admin/blacklist?act=editar&id={$b.id}">
                                                        <img src="{$tsConfig.default}/images/icons/editar.png" title="Editar" /></a>
                                                    <a href="#" onclick="admin.blacklist.borrar({$b.id}); return false">
                                                        <img src="{$tsConfig.url}/themes/default/images/icons/close.png" title="Eliminar"/></a>
                                                </td>
                                            </tr>{/foreach}
                                        </tbody>
                                        <tfoot>
				...
				<form action="" method="post" autocomplete="off">
					...
					<legend>{if $tsAct == 'editar'}Editar{else}Agregar{/if} bloqueo</legend>
							...
							<dt><label for="b_type">Tipo:</label></dt>
									...
									<dd>
                                                                        <textarea name="reason" id="b_reason" rows="3" cols="40">{$tsBL.reason}</textarea>
                                                                        </dd>
                                                                        </dl>
                                            {/if}
							<hr />
                                                    <p>
                                                 <input type="submit" name="{if $tsAct == 'editar'}edit{else}new{/if}" value="{if $tsAct == 'editar'}Guardar{else}Agregar{/if}" class="btn_g"/>
					</fieldset>
				</form>
			{/if}
</div>