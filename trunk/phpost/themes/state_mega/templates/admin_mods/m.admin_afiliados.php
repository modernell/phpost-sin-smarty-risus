                                ...
                                <table cellpadding="0" cellspacing="0" border="0" width="100%" align="center" class="admin_table">
                                        ...
                                        <tbody>{foreach from=$tsAfiliados item=af}
                                        	<tr id="few_{$af.aid}">
                                                <td>{$af.aid}</td>
                                                <td>
                                                    <a href="{$af.a_url}" id="a_url_{$af.aid}" target="_blank">
                                                        <span id="a_name_{$af.aid}">{$af.a_titulo}</span>
                                                    </a>
                                                </td>
                                                <td>{$af.a_date|hace}</td>
                                                <td>{$af.a_hits_in}</td>
                                                <td>{$af.a_hits_out}</td>
						<td id="status_afiliado_{$af.aid}">
                                                    {if $af.a_active == 0}
                                                    <font color="purple">Inactivo</font>{else}<font color="green">Activo</font>
                                                    {/if}
                                                </td>
                                                <td class="admin_actions">
                                                    <a href="afs/editar/{$af.aid}">
                                                        <img src="{$tsConfig.url}/themes/default/images/icons/editar.png" title="Editar"/>
                                                    </a>
                                                    <a href="#" onclick="ad_afiliado.detalles({$af.aid}); return false;">
                                                        <img src="{$tsConfig.url}/themes/default/images/icons/details.png" title="Detalles"/>
                                                    </a>
								<a onclick="admin.afs.accion({$af.aid}); return false">
                                                                  <img src="{$tsConfig.default}/images/reactivar.png" title="Activar/Desactivar Afiliado" />
                                                                </a>
                                                    <a href="#" onclick="admin.afs.borrar({$af.aid}); return false">
