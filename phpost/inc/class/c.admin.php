...
class tsAdmin
{
    ...
    */
    function getAdmins()
    {
        ...
        //
        $query = mysql_query('SELECT `user_id`, `user_name` FROM `u_miembros` WHERE user_rango = \'1\' ORDER BY user_id');
        //
    ...
    function getInst()
    {
        ...
        //
        $query = mysql_query('SELECT `stats_time_foundation`, `stats_time_upgrade` FROM `w_stats` WHERE stats_no = \'1\'');
        //
    ...
    */
    function saveConfig()
    {
        ...
        // UPDATE
        if (mysql_query('UPDATE `w_configuracion` SET `titulo` = \'' . $c['titulo'] . '\', `slogan` = \'' .
    ...
    */
    function getNoticias()
    {
        ...
        //
        $query = mysql_query('SELECT u.user_id, u.user_name, n.* FROM w_noticias AS n LEFT JOIN u_miembros AS u ON n.not_autor = u.user_id	WHERE n.not_id > \'0\' ORDER BY n.not_id DESC');
        $data = result_array($query);
    ...
    */
    ...
    {
        $not_id = $_GET['nid'];
        if (!mysql_num_rows(mysql_query('SELECT `not_id` FROM `w_noticias` WHERE `not_id` = \'' .
            (int)$not_id . '\' LIMIT 1')))
        {
            return 'El id ingresado no existe.';
        }
        mysql_query('DELETE FROM `w_noticias` WHERE `not_id` = \'' . (int)$not_id . '\'');
    }
    /*
    getNoticia()
    */
    function getNoticia()
    {
        ...
        //
        $query = mysql_query('SELECT `not_id`, `not_body`, `not_date`, `not_active` FROM w_noticias WHERE not_id = \'' .
            (int)$not_id . '\' LIMIT 1');
    ...
    */
    function newNoticia()
    {
        ...
        if (!empty($not_body))
        {
            if (mysql_query('INSERT INTO `w_noticias` (`not_body`, `not_autor`, `not_date`, `not_active`) VALUES (\'' .
    ...
    */
    function editNoticia()
    {
        ...
        if (!empty($not_body))
        {
            if (mysql_query('UPDATE `w_noticias` SET `not_autor` = \'' . $tsUser->uid . '\', `not_body` = \'' .
    ...
    */
    function getTemas()
    {
        global $tsCore;
        //
        $query = mysql_query('SELECT `tid`, `t_name`, `t_url`, `t_path` FROM `w_temas` WHERE tid != \'0\'');
        //
        $data = result_array($query);
    ...
    */
    function getTema()
    {
        ...
        //
        $query = mysql_query('SELECT `tid`, `t_name`, `t_url`, `t_path` FROM `w_temas` WHERE tid = \'' .
            (int)$tema_id . '\' LIMIT 1');
        $data = mysql_fetch_assoc($query);
    ...
    */
    function saveTema()
    {
        ...
        //
        if (mysql_query('UPDATE `w_temas` SET t_url = \'' . $t['url'] . '\', t_path = \'' .
    ...
    */
    function changeTema()
    {
        ...
        if (!empty($tema['tid']))
        {
            mysql_query('UPDATE `w_configuracion` SET tema_id = \'' . (int)$tema['tid'] . '\' WHERE tscript_id = \'1\'');
    ...
    */
    function deleteTema()
    {
        ...
        if (!empty($tema['tid']))
        {
            mysql_query('DELETE FROM `w_temas` WHERE tid = \'' . (int)$tema['tid'] . '\'');
            return true;
        } else
            return false;
    }
    ...
    */
    function newTema()
    {
        ...
        // NUEVO
        if (mysql_query('INSERT INTO `w_temas` (`t_name`, `t_url`, `t_path`, `t_copy`) VALUES (\'' .
    ...
    */
    function saveAds()
    {
        ...
        $a = array(
            ...
            'sid' => $_POST['adSearch']);
        //
        if (mysql_query('UPDATE `w_configuracion` SET ads_300 = \'' . $tsCore->
    ...
    */
    function saveOrden()
    {
        ...
        foreach ($subcats as $key => $cid)
        {
            if (!empty($cid))
            {
                mysql_query('UPDATE `p_categorias` SET c_orden = \'' . (int)$orden . '\' WHERE cid = \'' .
                    (int)$cid . '\'');
                $orden++;
            }
        }
    ...
    */
    function saveCat()
    {
        ...
        if (mysql_query('UPDATE `p_categorias` SET c_nombre = \'' . $tsCore->setSecure($c_nombre) .
    ...
    */
    function MoveCat()
    {
        if (mysql_query('UPDATE `p_posts` SET post_category = \'' . (int)$_POST['newcid'] .
    ...
    */
    function newCat()
    {
        ...
        $cimg = $tsCore->setSecure($tsCore->parseBadWords($_POST['c_img']));
        // ORDEN
        $query = mysql_query('SELECT COUNT(cid) AS total FROM `p_categorias`');
        $orden = mysql_fetch_assoc($query);
        $orden = $orden['total'] + 1;
        // INSERTS
        if (mysql_query('INSERT INTO `p_categorias` (`c_orden`, `c_nombre`, `c_seo`, `c_img`) VALUES (\'' .
    ...
    */
    function delCat()
    {
        ...
        {
            if (mysql_query('UPDATE `p_posts` SET post_category = \'' . (int)$ncid . '\' WHERE post_category = \'' .
                (int)$cid . '\''))
            {
                if (mysql_query('DELETE FROM `p_categorias` WHERE cid = \'' . (int)$cid . '\''))
                    return 1;
    ...
    */
    function getRangos()
    {
        global $tsCore;
        // RANGOS SIN PUNTOS
        $query = mysql_query('SELECT * FROM u_rangos ORDER BY rango_id, r_cant');
        // ARMAR ARRAY
    ...
    */
    function getRango()
    {
        global $tsCore;
        //
        $query = mysql_query('SELECT * FROM u_rangos WHERE rango_id = \'' . (int)$_GET['rid'] .
            '\' LIMIT 1');
        $data = mysql_fetch_assoc($query);
    ...
    */
    function getRangoUsers()
    {
        ...
        $query = mysql_query('SELECT u.user_id, u.user_name, u.user_email, u.user_registro, u.user_lastlogin FROM u_miembros AS u WHERE u.' .
            $where . ' LIMIT ' . $limit);
        //
        $data['data'] = result_array($query);

        // PAGINAS
        $query = mysql_query('SELECT COUNT(*) FROM u_miembros WHERE ' . $where);
    ...
    */
    function saveRango()
    {
        ...
        //
        if (mysql_query('UPDATE `u_rangos` SET r_name = \'' . $tsCore->setSecure($r['name']) .
    ...
    */
    function newRango()
    {
        ...
        if (mysql_query('INSERT INTO `u_rangos` (`r_name`, `r_color`, `r_image`, `r_cant`, `r_allows`, `r_type`) VALUES (\'' .
    ...
    */
    function delRango()
    {
        ...
        if ($rid > 3)
        {
            if (mysql_query('UPDATE u_miembros SET user_rango = \'' . (int)$_POST['new_rango'] .
                '\' WHERE user_rango = \'' . (int)$rid . '\''))
            {
                if (mysql_query('DELETE FROM u_rangos WHERE rango_id = \'' . (int)$rid . '\''))
                    return true;
            }
        } else
    ...
    */
    function SetDefaultRango()
    {
        ...
        $rid = $tsCore->setSecure($_GET['rid']);
        //
        $dato = mysql_fetch_assoc(mysql_query('SELECT rango_id, r_type FROM u_rangos WHERE rango_id = \'' .
            (int)$rid . '\' LIMIT 1'));
        if (!empty($dato['rango_id']) && $dato['r_type'] == 0)
        {
            if (mysql_query('UPDATE w_configuracion SET c_reg_rango = \'' . (int)$rid . '\' WHERE tscript_id = \'1\''))
                return true;
        } else
    ...
    */
    function getUsuarios()
    {
        ...
        //
        $query = mysql_query('SELECT u.*, r.*, p.* FROM u_perfil AS p LEFT JOIN u_miembros AS u ON u.user_id = p.user_id LEFT JOIN u_rangos AS r ON r.rango_id = u.user_rango ORDER BY ' .
            $order . ' ' . ($_GET['m'] == 'a' ? 'ASC' : 'DESC') . ' LIMIT ' . $limit);
        //
        $data['data'] = result_array($query);

        // PAGINAS
        $query = mysql_query('SELECT COUNT(*) FROM u_miembros WHERE user_id > \'0\'');
        list($total) = mysql_fetch_row($query);
    ...
    */
    function getUserPrivacidad()
    {
        global $tsCore;
        //
        $query = mysql_query('SELECT p_configs FROM u_perfil WHERE user_id = \'' . (int)
            $_GET['uid'] . '\' LIMIT 1');
        $data = mysql_fetch_assoc($query);
    ...
    */
    function setUserPrivacidad()
    {
        ...
        if (mysql_query('UPDATE u_perfil SET ' . $updates . ' WHERE user_id = \'' . (int)
            $_GET['uid'] . '\''))
            return true;
    ...
    */
    function getUserData()
    {
        ...
        //
        $query = mysql_query('SELECT u.*, r.*, p.* FROM u_perfil AS p LEFT JOIN u_miembros AS u ON u.user_id = p.user_id LEFT JOIN u_rangos AS r ON r.rango_id = u.user_rango WHERE u.user_id = \'' .
            (int)$user_id . '\' LIMIT 1');
        $data = mysql_fetch_assoc($query);
        $data['p_configs'] = unserialize($data['p_configs']);
        //
    ...
    */
    function setUserData($user_id)
    {
        global $tsCore;
        # DATA
        $query = mysql_query('SELECT `user_name`, `user_email`, `user_password` FROM u_miembros WHERE user_id = \'' .
            (int)$user_id . '\'');
        $data = mysql_fetch_assoc($query);
        # LOCALS
        ...
        if (mysql_query('UPDATE `u_miembros` SET user_email = \'' . $tsCore->setSecure($email) .
            ...
            ' WHERE user_id = \'' . (int)$user_id . '\''))
        {
            ...
            if ($_POST['sendata'])
                mail($email, 'Nuevos datos de acceso', 'Sus datos de acceso a ' . $tsCore->
                    ...
                    ', contraseña: ' . $password . '. Disculpe las molestias');
    ...
    function deleteContent($user_id){
        ...
        if(mysql_num_rows(mysql_query('SELECT user_id FROM u_miembros WHERE user_id = \''.$tsUser->uid.'\' && user_password = \''.md5(md5($_POST['password']).strtolower($tsUser->nick)).'\''))){
        $c = $_POST['bocuenta'];
        
        if($_POST['boposts'] || $c) mysql_query('DELETE FROM p_posts WHERE post_user = \''.$user_id.'\'');
        if($_POST['bofotos'] || $c) mysql_query('DELETE FROM f_fotos WHERE f_user = \''.$user_id.'\'');
        if($_POST['boestados'] || $c) mysql_query('DELETE FROM u_muro WHERE p_user_pub = \''.$user_id.'\'');
        if($_POST['bocomposts'] || $c) mysql_query('DELETE FROM p_comentarios WHERE c_user = \''.$user_id.'\'');
        if($_POST['bocomfotos'] || $c) mysql_query('DELETE FROM f_comentarios WHERE c_user = \''.$user_id.'\'');
        if($_POST['bocomestados'] || $c) mysql_query('DELETE FROM u_muro_comentarios WHERE c_user = \''.$user_id.'\'');
        if($_POST['bolikes'] || $c) mysql_query('DELETE FROM u_muro_likes WHERE user_id = \''.$user_id.'\'');
        if($_POST['boseguidores'] || $c) mysql_query('DELETE FROM u_follows WHERE f_id = \''.$user_id.'\' && f_type = \'1\'');
        if($_POST['bosiguiendo'] || $c) mysql_query('DELETE FROM u_follows WHERE f_user = \''.$user_id.'\' && f_type = \'1\'');
        if($_POST['bofavoritos'] || $c) mysql_query('DELETE FROM p_favoritos WHERE fav_user_id = \''.$user_id.'\'');
        if($_POST['bovotosposts'] || $c) mysql_query('DELETE FROM p_votos WHERE tuser = \''.$user_id.'\'');
        if($_POST['bovotosfotos'] || $c) mysql_query('DELETE FROM f_votos WHERE v_user = \''.$user_id.'\'');
        if($_POST['boactividad'] || $c) mysql_query('DELETE FROM u_actividad WHERE user_id = \''.$user_id.'\'');
        if($_POST['boavisos'] || $c) mysql_query('DELETE FROM u_avisos WHERE user_id = \''.$user_id.'\'');
        if($_POST['bobloqueos'] || $c) mysql_query('DELETE FROM u_bloqueos WHERE b_user = \''.$user_id.'\'');
        if($_POST['bomensajes'] || $c) mysql_query('DELETE FROM u_mensajes WHERE mp_from = \''.$user_id.'\'');  mysql_query('DELETE FROM u_respuestas WHERE mr_from = \''.$user_id.'\'');
        if($_POST['bosesiones'] || $c) mysql_query('DELETE FROM u_sessions WHERE session_user_id = \''.$user_id.'\'');
        if($_POST['bovisitas'] || $c) mysql_query('DELETE FROM w_visitas WHERE user = \''.$user_id.'\'');
        
        $data = mysql_fetch_row(mysql_query('SELECT user_name FROM u_miembros WHERE user_id = \''.$user_id.'\''));
        $admin = mysql_fetch_row(mysql_query('SELECT user_email FROM u_miembros WHERE user_id = \'1\''));
        
        if($c && $tsUser->uid != $user_id){
            mysql_query('DELETE FROM u_miembros WHERE user_id = \''.$user_id.'\'');
            mysql_query('DELETE FROM u_perfil WHERE user_id = \''.$user_id.'\'');
            mysql_query('DELETE FROM u_portal WHERE user_id = \''.$user_id.'\'');
            mysql_query('DELETE FROM w_denuncias WHERE d_user = \''.$user_id.'\'');
            mysql_query('DELETE FROM u_bloqueos WHERE b_auser = \''.$user_id.'\'');
            mysql_query('DELETE FROM u_mensajes WHERE mp_to = \''.$user_id.'\'');
            mysql_query('DELETE FROM w_visitas WHERE for = \''.$user_id.'\' && type = \'1\'');
        }
        
        mysql_query('INSERT INTO `u_avisos` (`user_id`, `av_subject`, `av_body`, `av_date`, `av_read`, `av_type`) VALUES (\'1\', \'Contenido eliminado\', \'Hola, le informamos que el administrador '.$tsUser->nick.' ('.$tsUser->uid.') ha eliminado '.($c ? 'la cuenta' : 'varios contenidos').' de '.$data[0].'.\', \''.time().'\', \'0\', \'1\')');
    ...
    */
    function getUserRango($user_id)
    {
        ...
        # CONSULTA
        $query = mysql_query('SELECT u.user_rango, r.rango_id, r.r_name, r.r_color FROM u_miembros AS u LEFT JOIN u_rangos AS r ON u.user_rango = r.rango_id WHERE u.user_id = \'' .
            (int)$user_id . '\' LIMIT 1');
        $data['user'] = mysql_fetch_assoc($query);

        # RANGOS DISPONIBLES
        $query = mysql_query('SELECT `rango_id`, `r_name`, `r_color` FROM `u_rangos`');
    ...
    */
    function setUserRango($user_id)
    {
        ...
        {
            if (mysql_query('UPDATE u_miembros SET user_rango = \'' . (int)$new_rango . '\' WHERE user_id = \'' .
                (int)$user_id . '\''))
                return true;
        }
    }
    ...
    function setUserInActivo()
    {
        ...
        $query = mysql_query('SELECT user_activo FROM u_miembros WHERE user_id = \'' . (int)
            $usuario . '\'');
        $data = mysql_fetch_assoc($query);
        ...
        {
            if (mysql_query('UPDATE u_miembros SET user_activo = \'0\' WHERE user_id = \'' .
                (int)$usuario . '\''))
            {
        ...
        {
            if (mysql_query('UPDATE u_miembros SET user_activo = \'1\' WHERE user_id = \'' .
                (int)$usuario . '\''))
            {
    ...
    function delSession()
    {
        ...
        if (mysql_num_rows(mysql_query('SELECT session_id FROM u_sessions WHERE session_id = \'' .
            $tsCore->setSecure($session_id) . '\' LIMIT 1')))
        {
            if (mysql_query('DELETE FROM u_sessions WHERE session_id = \'' . $tsCore->
    ...
    function getChangeNicks()
    {
        ...
        $limit = $tsCore->setPageLimit($max, true);
        //
        $query = mysql_query('SELECT u.user_id, u.user_name, n.* FROM u_nicks AS n LEFT JOIN u_miembros AS u ON n.user_id = u.user_id WHERE estado = \'0\' ORDER BY n.time DESC LIMIT ' .
            $limit);
        //
        $data['data'] = result_array($query);

        // PAGINAS
        $query = mysql_query('SELECT COUNT(*) FROM u_nicks WHERE estado = \'0\'');
        list($total) = mysql_fetch_row($query);
    ...
    function ChangeNick_o_no()
    {
        ...
        $nick_id = $_POST['nid'];
        //
        $datos = mysql_fetch_assoc(mysql_query('SELECT * FROM u_nicks WHERE id = \'' . (int)
            $nick_id . '\' LIMIT 1'));
        //
        if ($_POST['accion'] == 'aprobar')
        {
            mysql_query('UPDATE u_miembros SET user_name = \'' . $datos['name_2'] . '\', user_password = \'' .
                $datos['hash'] . '\', user_name_changes = user_name_changes - 1 WHERE user_id = \'' .
                $datos['user_id'] . '\'');
            mysql_query('UPDATE u_nicks SET estado = \'1\' WHERE id = \'' . (int)$nick_id .
                '\'');
            // AVISO
        ...
        {
            mysql_query('UPDATE u_miembros SET user_name_changes = user_name_changes - 1 WHERE user_id = \'' .
                $datos['user_id'] . '\'');
            mysql_query('UPDATE u_nicks SET estado = \'2\' WHERE id = \'' . (int)$nick_id .
                '\'');
            // AVISO
    ...
    function GetAdminPosts()
    {
        ...
        //
        $query = mysql_query('SELECT u.user_id, u.user_name, c.c_nombre, c.c_seo, c.c_img, p.* FROM p_posts AS p LEFT JOIN u_miembros AS u ON p.post_user = u.user_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE p.post_id > \'0\' ORDER BY ' .
            $order . ' ' . ($_GET['m'] == 'a' ? 'ASC' : 'DESC') . ' LIMIT ' . $limit);
        //
        $data['data'] = result_array($query);

        // PAGINAS
        $query = mysql_query('SELECT COUNT(*) FROM p_posts WHERE post_id > \'0\'');
        list($total) = mysql_fetch_row($query);
    ...
    function GetAdminFotos()
    {
        ...
        $limit = $tsCore->setPageLimit($max, true);
        //
        $query = mysql_query('SELECT u.user_id, u.user_name, f.* FROM f_fotos AS f LEFT JOIN u_miembros AS u ON f.f_user = u.user_id WHERE f.foto_id > \'0\' ORDER BY f.foto_id DESC LIMIT ' .
            $limit);
        //
        $data['data'] = result_array($query);

        // PAGINAS
        $query = mysql_query('SELECT COUNT(*) FROM f_fotos WHERE foto_id > \'0\'');
        list($total) = mysql_fetch_row($query);
    ...
    function DelFoto()
    {
        ...
        if (mysql_num_rows(mysql_query('SELECT foto_id FROM `f_fotos` WHERE foto_id = \'' .
            (int)$foto . '\'')))
        {
            if (mysql_query('DELETE FROM f_fotos WHERE foto_id = \'' . (int)$foto . '\''))
            {
    ...
    function setOpenClosedFoto()
    {
        ...
        $query = mysql_query('SELECT f_closed FROM f_fotos WHERE foto_id = \'' . (int)$_POST['fid'] .
            '\'');
        $data = mysql_fetch_assoc($query);
        ...
        if ($data['f_closed'] == 1)
        {
            if (mysql_query('UPDATE f_fotos SET f_closed = \'0\' WHERE foto_id = \'' . (int)
                $_POST['fid'] . '\''))
            {
        ...
        {
            if (mysql_query('UPDATE f_fotos SET f_closed = \'1\' WHERE foto_id = \'' . (int)
                $_POST['fid'] . '\''))
            {
    ...
    function setShowHideFoto()
    {
        ...
        $query = mysql_query('SELECT f_status FROM f_fotos WHERE foto_id = \'' . (int)$_POST['fid'] .
            '\'');
        $data = mysql_fetch_assoc($query);
        ...
        if ($data['f_status'] == 1)
        {
            if (mysql_query('UPDATE f_fotos SET f_status = \'0\' WHERE foto_id = \'' . (int)
                $_POST['fid'] . '\''))
            {
        ...
        {
            if (mysql_query('UPDATE f_fotos SET f_status = \'1\' WHERE foto_id = \'' . (int)
                $_POST['fid'] . '\''))
            {
    ...
    function setNoticiaInActive()
    {
        ...
        $query = mysql_query('SELECT not_active FROM w_noticias WHERE not_id = \'' . (int)
            $noticia . '\'');
        $data = mysql_fetch_assoc($query);
        ...
        {
            if (mysql_query('UPDATE w_noticias SET not_active = \'0\' WHERE not_id = \'' . (int)
                $noticia . '\''))
            {
        ...
        {
            if (mysql_query('UPDATE w_noticias SET not_active = \'1\' WHERE not_id = \'' . (int)
                $noticia . '\''))
            {
    ...
    function getBlackList()
    {
        ...
        $limit = $tsCore->setPageLimit($max, true);
        //
        $query = mysql_query('SELECT u.user_id, u.user_name, b.* FROM w_blacklist AS b LEFT JOIN u_miembros AS u ON b.author = u.user_id ORDER BY b.date DESC LIMIT ' .
            $limit);
        //
        $data['data'] = result_array($query);

        // PAGINAS
        $query = mysql_query('SELECT COUNT(*) FROM w_blacklist');
        list($total) = mysql_fetch_row($query);
    ...
    function getBlock()
    {
        return mysql_fetch_assoc(mysql_query('SELECT type, value, reason FROM w_blacklist WHERE id = \'' .
            (int)$_GET['id'] . '\' LIMIT 1'));
    }
    ...
    function saveBlock()
    {
        ...
        } else
        {
            ...
            if (!mysql_num_rows(mysql_query('SELECT id FROM w_blacklist WHERE type = \'' . (int)
                $_POST['type'] . '\' && value = \'' . $tsCore->setSecure($_POST['value']) . '\'')))
            {
                if (mysql_query('UPDATE w_blacklist SET type = \'' . (int)$_POST['type'] . '\', value = \'' .
    ...
    function newBlock()
    {
        ...
        } else
        {
            ...
            if (!mysql_num_rows(mysql_query('SELECT id FROM w_blacklist WHERE type = \'' . (int)
                $_POST['type'] . '\' && value = \'' . $tsCore->setSecure($_POST['value']) . '\'')))
            {
                if (mysql_query('INSERT INTO w_blacklist (type, value, reason, author, date) VALUES (\'' .
    ...
    function deleteBlock()
    {
        ...
        if (mysql_query('DELETE FROM w_blacklist WHERE id = \'' . (int)$_POST['bid'] . '\''))
    ...
    function getBadWords()
    {
        ...
        $limit = $tsCore->setPageLimit($max, true);
        //
        $query = mysql_query('SELECT u.user_id, u.user_name, bw.* FROM w_badwords AS bw LEFT JOIN u_miembros AS u ON bw.author = u.user_id ORDER BY bw.wid DESC LIMIT ' .
            $limit);
        //
        $data['data'] = result_array($query);

        // PAGINAS
        $query = mysql_query('SELECT COUNT(*) FROM w_badwords');
        list($total) = mysql_fetch_row($query);
    ...
    function getBadWord()
    {
        return mysql_fetch_assoc(mysql_query('SELECT * FROM w_badwords WHERE wid = \'' .
            (int)$_GET['id'] . '\' LIMIT 1'));
    }
    ...
    function saveBadWord()
    {
        ...
        {
            if (!mysql_num_rows(mysql_query('SELECT wid FROM w_badwords WHERE LOWER(before) = \'' .
                $tsCore->setSecure(strtolower($_POST['before'])) . '\' && LOWER(after) = \'' . $tsCore->
                setSecure(strtolower($_POST['after'])) . '\'')))
            {
                if (mysql_query('UPDATE `w_badwords` SET method = \'' . $method . '\', type = \'' .
    ...
    function newBadWord()
    {
        ...
        } else
        {
            if (!mysql_num_rows(mysql_query('SELECT wid FROM w_badwords WHERE LOWER(before) = \'' .
                ...
                setSecure(strtolower($_POST['after'])) . '\'')))
            {
                if (mysql_query('INSERT INTO w_badwords (word, swop, method, type, author, reason, date) VALUES (\'' .
    ...
    function deleteBadWord()
    {
        ...
        if (mysql_query('DELETE FROM w_badwords WHERE wid = \'' . (int)$_POST['wid'] . '\''))
