...
class tsCuenta {
	...
	public function loadPerfil($user_id = 0){
		...
		//
		$query = mysql_query('SELECT p.*, u.user_registro, u.user_lastactive FROM u_perfil AS p LEFT JOIN u_miembros AS u ON p.user_id = u.user_id WHERE p.user_id = \''.(int)$user_id.'\' LIMIT 1');
	...
	*/
	function loadHeadInfo($user_id){
		...
		$visitado = mysql_num_rows(mysql_query('SELECT id FROM `w_visitas` WHERE `for` = \''.(int)$user_id.'\' && `type` = \'1\' && '.($tsUser->is_member ? '(`user` = \''.$tsUser->uid.'\' OR `ip` LIKE \''.$_SERVER['REMOTE_ADDR'].'\')' : '`ip` LIKE \''.$_SERVER['REMOTE_ADDR'].'\'').' LIMIT 1'));
		if(($tsUser->is_member && $visitado == 0 && $tsUser->uid != $user_id) || ($tsCore->settings['c_hits_guest'] == 1 && !$tsUser->is_member && !$visitado)) {
			mysql_query('INSERT INTO w_visitas (`user`, `for`, `type`, `date`, `ip`) VALUES (\''.$tsUser->uid.'\', \''.(int)$user_id.'\', \'1\', \''.time().'\', \''.$_SERVER['REMOTE_ADDR'].'\')');
			mysql_query('UPDATE u_perfil SET p_visitas = p_visitas + 1 WHERE user_id = \''.(int)$user_id.'\'');
		}else{
		mysql_query('UPDATE `w_visitas` SET `date` = \''.time().'\', ip = \''.$_SERVER['REMOTE_ADDR'].'\' WHERE `for` = \''.(int)$post_id.'\' && `type` = \'1\'');
		}
		
		// REAL STATS
		$data['stats'] = mysql_fetch_assoc(mysql_query('SELECT u.user_id, u.user_rango, u.user_puntos, u.user_posts, u.user_comentarios, u.user_seguidores, u.user_cache, r.r_name, r.r_color FROM u_miembros AS u LEFT JOIN u_rangos AS r ON  u.user_rango = r.rango_id WHERE u.user_id = \''.(int)$user_id.'\''));
		
        if($data['stats']['user_cache'] < time()-($tsCore->settings['c_stats_cache']*60)){
        $q1 = mysql_fetch_row(mysql_query('SELECT COUNT(post_id) AS p FROM p_posts WHERE post_user = \''.(int)$user_id.'\' && post_status = \'0\''));
        $q2 = mysql_fetch_row(mysql_query('SELECT COUNT(follow_id) AS s FROM u_follows WHERE f_id =\''.(int)$user_id.'\' && f_type = \'1\''));
        ...
        mysql_query('UPDATE u_miembros SET user_posts = \''.$q1[0].'\', user_comentarios = \''.$q3[0].'\', user_seguidores = \''.$q2[0].'\', user_cache = \''.time().'\' WHERE  user_id = \''.(int)$user_id.'\'');
        }
        $q4 = mysql_fetch_row(mysql_query('SELECT COUNT(foto_id) AS f FROM f_fotos WHERE f_user = \''.(int)$user_id.'\' && f_status = \'0\''));
        $data['stats']['user_fotos'] = $q4[0];
		
		// BLOQUEADO
		$query = mysql_query('SELECT * FROM `u_bloqueos` WHERE b_user = \''.$tsUser->uid.'\' AND b_auser = \''.(int)$user_id.'\' LIMIT 1');
        $data['block'] = mysql_fetch_assoc($query);
	...
	*/
	function loadGeneral($user_id){
		global $tsCore;
		// MEDALLAS
		$query = mysql_query('SELECT m.*, a.* FROM w_medallas AS m LEFT JOIN w_medallas_assign AS a ON a.medal_id = m.medal_id WHERE a.medal_for = \''.(int)$user_id.'\' AND m.m_type = \'1\' ORDER BY a.medal_date DESC LIMIT 21');
		$data['medallas'] = result_array($query);
        $data['m_total'] = count($data['medallas']);
        
		// SEGUIDORES
        $query = mysql_query('SELECT f.follow_id, u.user_id, u.user_name FROM u_follows AS f LEFT JOIN u_miembros AS u ON f.f_user = u.user_id WHERE f.f_id = \''.(int)$user_id.'\' && f.f_type = \'1\' && u.user_activo = \'1\' && u.user_baneado = \'0\' ORDER BY f.f_date DESC LIMIT 21');
        $data['segs']['data'] = result_array($query);
        $data['segs']['total'] = count($data['segs']['data']);
        
		// SIGUIENDO
        $query = mysql_query('SELECT f.follow_id, u.user_id, u.user_name FROM u_follows AS f LEFT JOIN u_miembros AS u ON f.f_id = u.user_id WHERE f.f_user = \''.(int)$user_id.'\' AND f.f_type = \'1\' && u.user_activo = \'1\' && u.user_baneado = \'0\' ORDER BY f.f_date DESC LIMIT 21');
    ...
    */
    function iFollow($user_id){
        global $tsUser;
        // SEGUIR
        $query = mysql_query('SELECT follow_id FROM u_follows WHERE f_id = \''.(int)$user_id.'\' AND f_user = \''.(int)$tsUser->uid.'\' AND f_type = \'1\' LIMIT 1');
    ...
    */
    function yFollow($user_id){
        global $tsUser;
        // YO LE SIGO?
        $query = mysql_query('SELECT follow_id FROM u_follows WHERE f_id = \''.(int)$tsUser->uid.'\' AND f_user = \''.(int)$user_id.'\' AND f_type = \'1\' LIMIT 1');
		$data = mysql_num_rows($query);
    ...
    */
    function loadPosts($user_id){
        global $tsUser;
        //
        $query = mysql_query('SELECT p.post_id, p.post_title, p.post_puntos, c.c_seo, c.c_img FROM p_posts AS p LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE p.post_status = \'0\' AND p.post_user = \''.(int)$user_id.'\' ORDER BY p.post_date DESC LIMIT 18');
    ...
    */
    function loadMedallas($user_id){
        //
		$query = mysql_query('SELECT m.*, a.* FROM w_medallas AS m LEFT JOIN w_medallas_assign AS a ON a.medal_id = m.medal_id WHERE a.medal_for = \''.(int)$user_id.'\' AND m.m_type = \'1\' ORDER BY a.medal_date DESC');
		$data['medallas'] = result_array($query);
	...
	function savePerfil(){
		...
		switch($save){
			case 1:
                ...
                $year = date("Y",time());
                // ANTIGUOS DATOS
				$query = mysql_query('SELECT user_dia, user_mes, user_ano, user_pais, user_estado, user_sexo, user_firma FROM U_perfil WHERE user_id = \''.$tsUser->uid.'\' LIMIT 1');
				...
				elseif($tsUser->info['user_email'] != $perfilData['email']) {
				    $query = mysql_query('SELECT user_id FROM u_miembros WHERE user_email = \''.$tsCore->setSecure($perfilData['email']).'\' LIMIT 1');
            ...
            case 6:
                ...
                elseif($new_passwd != $confirm_passwd) return array('error' => 'Tu nueva contrase&ntilde;a debe ser igual a la confirmaci&oacute;n de la misma.');
                    ...
                    if($key != $tsUser->info['user_password']) return array('error' => 'Tu contrase&ntilde;a actual no es correcta.');
						...
						if(mysql_query('UPDATE u_miembros SET user_password = \''.$tsCore->setSecure($new_key).'\' WHERE user_id = \''.$tsUser->uid.'\'')) return true;
                    }
                }
            break;
            case 7:
			...
			case 8: //2678400 es un mes :)
                ...
                if(mysql_num_rows(mysql_query('SELECT id FROM w_blacklist WHERE type = \'4\' && LOWER(value) = \''.$tsCore->setSecure($nuevo_nick).'\' LIMIT 1'))) return array('error' => 'Nick no permitido');
                if(mysql_num_rows(mysql_query('SELECT user_id FROM u_miembros WHERE LOWER(user_name) = \''.$tsCore->setSecure($nuevo_nick).'\' LIMIT 1'))) return array('error' => 'Nombre en uso');
				$data = mysql_fetch_assoc(mysql_query('SELECT id, user_id, time FROM u_nicks WHERE user_id = \''.$tsUser->uid.'\' AND estado = 0 LIMIT 1'));
				if(!empty($data['id'])) return array('error' => 'Ya tiene una petici&oacute;n  de cambio en curso');
				elseif(time() - $data['time'] >= 31536000) mysql_query('UPDATE u_miembros SET user_name_changes = \'3\' WHERE user_id = \''.$data['user_id'].'\'');
				...
				if(mysql_query('INSERT INTO `u_nicks` (`user_id`, `user_email`, `name_1`, `name_2`, `hash`, `time`, `ip`) VALUES (\''.$tsUser->uid.'\', \''.$tsCore->setSecure($email).'\', \''.$tsUser->nick.'\', \''.$tsCore->setSecure($nuevo_nick).'\', \''.$key.'\', \''.time().'\', \''.$tsCore->setSecure($_SERVER['REMOTE_ADDR']).'\')')) return array('error' => 'Proceso iniciado, recibir&aacute; la respuesta en el correo indicado cuando valoremos el cambio.');
				}
            break;
		}
        ...
        if($save > 1 && $save < 6){
			...
			$porcen = mysql_fetch_assoc(mysql_query('SELECT p_total FROM u_perfil WHERE user_id = \''.$tsUser->uid.'\' LIMIT 1'));
    		$porcen = unserialize($porcen['p_total']);
    		$porcen[$tid] = $total[$tid];
    		$porcenNow = $this->getPorcentVal($porcen);
    		$porcen = serialize($porcen);
			mysql_query('UPDATE u_perfil SET p_total = \''.$porcen.'\' WHERE user_id = \''.$tsUser->uid.'\'');
        }
		// ACTUALIZAR
		if($save == 1) {
		    mysql_query('UPDATE u_miembros SET user_email = \''.$tsCore->setSecure($perfilData['email'], true).'\' WHERE user_id = \''.$tsUser->uid.'\'');
            array_splice($perfilData, 0, 1); // HACK
            $updates = $tsCore->getIUP($perfilData, 'user_');
			if(!mysql_query('UPDATE u_perfil SET '.$updates.' WHERE user_id = \''.$tsUser->uid.'\'')) return array('error' => mysql_error());
		} else {
	...
	*/
	...
	function desCuenta() {
	global $tsUser, $tsCore;
	if(mysql_query('UPDATE u_miembros SET user_activo = \'0\' WHERE user_id = \''.$tsUser->uid.'\''))
	...
	*/
	function loadImages($user_id = 0){
		...
		//
		if(empty($user_id)) $user_id = $tsUser->uid;
		$images = result_array(mysql_query('SELECT * FROM u_fotos WHERE f_user = \''.(int)$user_id.'\''));
		//
		return $images;
	}
	/*
	...
	*/
	function addImagen(){
		...
		if(empty($img_url) || $img_url == 'http://') return array('field' => 'url', 'error' => 'Ingresa la URL de la imagen.');
		    ...
		    mysql_query('INSERT INTO u_fotos (f_user, f_url, f_caption) VALUES (\''.$tsUser->uid.'\', \''.$img_url.'\', \''.$img_cap.'\')');
			return array('id' => mysql_insert_id(), 'field' => '', 'error' => '');
		}
	}
	/*
		delImagen()
	*/
	function delImagen(){
		...
		// BORRANDO
		mysql_query('DELETE FROM u_fotos WHERE foto_id = \''.(int)$img_id.'\' AND f_user = \''.$tsUser->uid.'\'');
	}
	/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
							// MANEJAR BLOQUEOS \\
	/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
    function bloqueosCambiar(){
        ...
        if($exists && $tsUser->uid != $auser){
            if($bloquear == 1){
                // YA BLOQUEADO?
				$query = mysql_query('SELECT bid FROM u_bloqueos WHERE b_user = \''.$tsUser->uid.'\' AND b_auser = \''.(int)$auser.'\' LIMIT 1');
                $noexists = mysql_num_rows($query);
                
                // NO HA SIDO BLOQUEADO
                if(empty($noexists)) {
				    if(mysql_query('INSERT INTO u_bloqueos (b_user, b_auser) VALUES (\''.$tsUser->uid.'\', \''.(int)$auser.'\')'))
                    return "1: El usuario fue bloqueado satisfactoriamente."; 
                } else return '0: Ya has bloqueado a este usuario.';
                // 
            } else{
			    if(mysql_query('DELETE FROM u_bloqueos WHERE b_user = \''.$tsUser->uid.'\'  AND b_auser = \''.(int)$auser.'\''))
