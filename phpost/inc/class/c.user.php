...
class tsUser {
	...
	*/
	function actualizarPuntos()
    {
		...
		if($ultimaRecarga < $tiempoActual){
			...
			//
            mysql_query('UPDATE u_miembros SET user_puntosxdar = '.($tsCore->settings['c_keep_points'] == 0 ? $this->permisos['gopfd'] : 'user_puntosxdar + '.$this->permisos['gopfd']).', user_nextpuntos = '.$sigRecarga.' WHERE user_id = \''.$this->uid.'\'');
			// VAMONOS
			return true;
		}
	}
	...
	*/
	function DarMedalla(){
		//
        $q1 = mysql_num_rows(mysql_query('SELECT wm.medal_id FROM w_medallas AS wm LEFT JOIN w_medallas_assign AS wma ON wm.medal_id = wma.medal_id WHERE wm.m_type = \'1\' AND wma.medal_for = \''.$this->uid.'\''));        
		$q2 = mysql_fetch_row(mysql_query('SELECT COUNT(follow_id) AS f FROM u_follows WHERE f_id = \''.$this->uid.'\' && f_type = \'1\''));
        $q3 = mysql_fetch_row(mysql_query('SELECT COUNT(follow_id) AS s FROM u_follows WHERE f_user = \''.$this->uid.'\' && f_type = \'1\''));
        $q4 = mysql_fetch_row(mysql_query('SELECT COUNT(cid) AS c FROM p_comentarios WHERE c_user = \''.$this->uid.'\' && c.c_status = \'0\''));
        $q5 = mysql_fetch_row(mysql_query('SELECT COUNT(cid) AS cf FROM f_comentarios WHERE c_user = \''.$this->uid.'\''));
        $q6 = mysql_fetch_row(mysql_query('SELECT COUNT(foto_id) AS fo FROM f_fotos WHERE f_status = \'0\' && f_user = \''.$this->uid.'\''));
        $q7 = mysql_fetch_row(mysql_query('SELECT COUNT(post_id) AS p FROM p_posts WHERE post_user = \''.$this->uid.'\' && post_status = \'0\''));
        // MEDALLAS
		$datamedal = result_array($query = mysql_query('SELECT medal_id, m_cant, m_cond_user, m_cond_user_rango FROM w_medallas WHERE m_type = \'1\' ORDER BY m_cant DESC'));
		//		
		...
		if(!empty($newmedalla)){
		if(!mysql_num_rows(mysql_query('SELECT id FROM w_medallas_assign WHERE medal_id = \''.(int)$newmedalla.'\' && medal_for = \''.$this->uid.'\''))){
		mysql_query('INSERT INTO `w_medallas_assign` (`medal_id`, `medal_for`, `medal_date`, `medal_ip`) VALUES (\''.(int)$newmedalla.'\', \''.$this->uid.'\', \''.time().'\', \''.$_SERVER['REMOTE_ADDR'].'\')');
		mysql_query('INSERT INTO u_monitor (user_id, obj_uno, not_type, not_date) VALUES (\''.$this->uid.'\', \''.(int)$newmedalla.'\', \'15\', \''.time().'\')');
	...
	*/
	function loadUser($login = FALSE)
    {
        ...
        $this->info = mysql_fetch_assoc($query);
        // Existe el usuario?
	...
	$query = mysql_query("SELECT r_name, r_color, r_image, r_special, r_allows FROM u_rangos WHERE rango_id = '".$this->info['user_id']."' LIMIT 1");
        if ($query) //fix porque no devolvia un query lleno supongo porque el campo info user id estaba vacio
        $this->info['rango'] = mysql_fetch_assoc($query);
        
		// PERMISOS SEGUN RANGO
		$datis = mysql_fetch_assoc(mysql_query('SELECT r_allows FROM u_rangos WHERE rango_id = \''.$this->info['user_rango'].'\' LIMIT 1'));
		...
		// ULTIMA ACCION
		mysql_query('UPDATE u_miembros SET user_lastactive = \''.time().'\' WHERE user_id = \''.$this->uid.'\'');
        // Si ha iniciado sesión cargamos estos datos.
        if($login)
        {
            // Last login
			mysql_query('UPDATE u_miembros SET user_lastlogin = \''.$this->session->time_now.'\' WHERE user_id = \''.$this->uid.'\'');
            /* REGISTAR IP */
            mysql_query('UPDATE u_miembros SET user_last_ip = \''.$this->session->ip_address.'\' WHERE user_id = \''.$this->uid.'\'');
        }
	...
	*/
	function loginUser($username, $password, $remember = FALSE, $redirectTo = NULL){
		...
		/* CONSULTA */
		$query = mysql_query('SELECT user_id, user_password, user_pwtype, user_activo, user_baneado FROM u_miembros WHERE LOWER(user_name) = \''.$username.'\' LIMIT 1');
        if(!$query) $query = mysql_query('SELECT user_id, user_password, user_activo, user_baneado FROM u_miembros WHERE LOWER(user_name) = \''.$username.'\' LIMIT 1');
        //
        ...
        //
       	if($data['user_pwtype']){
            ...
            if(in_array($data['user_password'], $other_passwords)){
                // UPDATE
				mysql_query('UPDATE u_miembros SET user_password = \''.$tsCore->setSecure($pp_password).'\', user_pwtype = \'0\' WHERE user_id = '.$data['user_id'].'');
                //
	...
	*/
	function logoutUser($user_id, $redirectTo = '/'){
		...
		mysql_query('UPDATE u_miembros SET user_lastactive \''.$last_active.'\' WHERE user_id = \''.(int)$user_id.'\'');
		/* REDERIGIR */
		if($redirectTo != NULL) $tsCore->redirectTo($redirectTo);	// REDIRIGIR
		else return true;
	}
	...
	*/
	function userActivate($tsUserID = 0, $tsKey = 0){
		...
		//
		$query = mysql_query('SELECT user_name, user_password, user_registro FROM u_miembros WHERE user_id = \''.$tsUserID.'\' LIMIT 1');
		$tsData = mysql_fetch_assoc($query);	// CARGAMOS DATOS
		$tsKeyLocal = md5($tsData['user_registro']);
		//
		if(mysql_num_rows($query) == 0 || $tsKey != $tsKeyLocal){
			return false;
		} else {
		    if(mysql_query('UPDATE u_miembros SET user_activo = 1 WHERE user_id = \''.$tsUserID.'\'')) {
                return $tsData;
			}
			else return false;
		}
    ...
    */
    function getUserBanned(){
        //
		$query = mysql_query('SELECT * FROM u_suspension WHERE user_id = \''.$this->uid.'\' LIMIT 1');
        $data = mysql_fetch_assoc($query);
        ...
        if($data['susp_termina'] > 1 && $data['susp_termina'] < $now){
		    mysql_query('UPDATE u_miembros SET user_baneado = 0 WHERE user_id = \''.$this->uid.'\'');
			mysql_query('DELETE FROM u_suspension WHERE user_id = \''.$this->uid.'\'');
            return false;
        } else return $data;
    }
	/*
	...
	*/
	...
	global $tsCore;
		...
		$query = mysql_query('SELECT user_id FROM u_miembros WHERE LOWER(user_name) = \''.$tsCore->setSecure($tsUsername).'\' LIMIT 1');
		$tsUser = mysql_fetch_assoc($query);
    ...
    */
    function getUserName($user_id){
		//
		$query = mysql_query('SELECT user_name FROM u_miembros WHERE user_id = \''.(int)$user_id.'\' LIMIT 1');
		$tsUser = mysql_fetch_assoc($query);
    ...
    public function iFollow($user_id){
        # SIGO A ESTE USUARIO
        $query = mysql_query('SELECT follow_id FROM u_follows WHERE f_id = \''.(int)$user_id.'\' AND f_user = \''.$this->uid.'\' AND f_type = \'1\' LIMIT 1');
    ...
    public function getVCard($user_id){
		...
		//  STATS
		$query =  mysql_query('SELECT u.user_puntos, r.r_name, r.r_color, r.r_image FROM u_miembros AS u LEFT JOIN u_rangos AS r ON u.user_rango = r.rango_id WHERE user_id = \''.(int)$user_id.'\' LIMIT 1');
		...
		$q3 = mysql_fetch_row(mysql_query('SELECT COUNT(follow_id) AS s FROM u_follows WHERE f_id = \''.(int)$user_id.'\' && f_type = \'1\''));
    ...
    */
    function getUsuarios(){
        ...
        // TOTAL Y PAGINAS
        $query = mysql_query('SELECT COUNT(u.user_id) AS total FROM u_miembros AS u LEFT JOIN u_perfil AS p ON u.user_id = p.user_id WHERE u.user_activo = \'1\' && u.user_baneado = \'0\' '.$w_online.' '.$w_avatar.' '.$w_sex.' '.$w_pais.' '.$w_rango);
        $total = mysql_fetch_assoc($query);
        $total = $total['total'];
        
        $pages = $tsCore->getPagination($total, 12);
        // CONSULTA
        $query = mysql_query('SELECT u.user_id, u.user_name, p.user_pais, p.user_sexo, p.p_avatar, p.p_mensaje, u.user_rango, u.user_puntos, u.user_comentarios, u.user_posts, u.user_lastactive, u.user_baneado, r.r_name, r.r_color, r.r_image FROM u_miembros AS u LEFT JOIN u_perfil AS p ON u.user_id = p.user_id LEFT JOIN u_rangos AS r ON r.rango_id = u.user_rango WHERE u.user_activo = \'1\' && u.user_baneado = \'0\' '.$w_online.' '.$w_avatar.' '.$w_sex.' '.$w_pais.' '.$w_rango.'  ORDER BY u.user_id DESC LIMIT '.$pages['limit']);
...
class tsSession {
	 ...
	 */
     function read()
     {
		...
		$query = mysql_query('SELECT * FROM u_sessions WHERE session_id = \''.$this->ID.'\'');
	...
	function create()
	{
		...
		mysql_query('INSERT INTO u_sessions (session_id, session_user_id, session_ip, session_time) VALUES (\''.$this->ID.'\', \'0\', \''.$this->ip_address.'\', \''.$this->time_now.'\') ');
	...
	function update($user_id = 0, $autologin = FALSE, $force_update = FALSE)
	{
	   ...
	   mysql_query('UPDATE u_sessions SET session_user_id = \''.$this->userdata['session_user_id'].'\', session_ip = \''.$this->userdata['session_ip'].'\', session_time = \''.$this->userdata['session_time'].'\', session_autologin = \''.$this->userdata['session_autologin'].'\' WHERE session_id = \''.$this->ID.'\'');
	...
	{
	   // Elminar de la DB
       mysql_query('DELETE FROM u_sessions WHERE session_id = '.$this->ID.'');
	   // Reset a la cookie
	...
	function sess_gc()
	{
		...
		if ((rand() % 100) < 30)
		{
			...
			mysql_query('DELETE FROM u_sessions WHERE session_time < '.$expire.' AND session_autologin = \'0\'');
        }
	}
}