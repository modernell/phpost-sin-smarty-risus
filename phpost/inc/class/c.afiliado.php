...
class tsAfiliado {
	...
	*/
    function getAfiliados($type = 'home'){
        ...
        //
        if($type == 'home')
		$query = mysql_query('SELECT aid,a_titulo,a_url,a_banner,a_descripcion FROM w_afiliados WHERE a_active = \'1\' ORDER BY RAND() LIMIT 5');
        elseif($type == 'admin')
        $query = mysql_query('SELECT aid,a_titulo,a_url,a_banner,a_descripcion,a_sid,a_hits_in,a_hits_out,a_date,a_active FROM w_afiliados');   
        //
    ...
    */
    function getAfiliado($type){
        ...
        //
        if(!$type){
		$query = mysql_query('SELECT aid,a_titulo,a_url,a_banner,a_descripcion FROM w_afiliados WHERE aid = \''.(int)$_POST['ref'].'\' ');
		}elseif($type = 'admin'){
		$query = mysql_query('SELECT aid,a_titulo,a_url,a_banner,a_descripcion FROM w_afiliados WHERE aid = \''.(int)$_GET['aid'].'\' ');
		}
    ...
    */
    function newAfiliado(){
        ...
        //
		if(mysql_query('INSERT INTO `w_afiliados` (a_titulo, a_url, a_banner, a_descripcion, a_sid, a_date) VALUES (\''.$tsCore->setSecure($dataIn['titulo']).'\', \''.$tsCore->setSecure($dataIn['url']).'\', \''.$tsCore->setSecure($dataIn['banner']).'\', \''.$tsCore->setSecure($dataIn['desc']).'\', \''.intval($dataIn['sid']).'\',\''.time().'\')')) {
		$afid = mysql_insert_id();
		  // AVISO
	...
	function EditarAfiliado(){
	   ...
	   if(!$afiliado || !$titulo || !$url || !$banner || !$descripcion){
        ...
        //
		if(mysql_query('UPDATE w_afiliados SET a_titulo = \''.$tsCore->setSecure($titulo).'\', a_url = \''.$tsCore->setSecure($url).'\', a_banner = \''.$tsCore->setSecure($banner).'\', a_descripcion = \''.$tsCore->setSecure($descripcion).'\' WHERE aid= \''.(int)$afiliado.'\'')) {
		return '1: Guardado';
        }else{
	...
	function DeleteAfiliado($aid){
		...
		if(mysql_query('DELETE FROM w_afiliados WHERE aid = \''.(int)$aid.'\''));
	...
	function SetActionAfiliado(){
        ...
        $query = mysql_query('SELECT a_active FROM w_afiliados WHERE aid = \''.(int)$afiliado.'\'');
        $data = mysql_fetch_assoc($query);
        
		
        if($data['a_active'] == 1){
		    if(mysql_query('UPDATE w_afiliados SET a_active = \'0\' WHERE aid = \''.(int)$afiliado.'\'')) {
			   return '2: Afiliado deshabilitado';
			   }else return '0: Ocurri&oacute, un error';
        } else{
		    if(mysql_query('UPDATE w_afiliados SET a_active = \'1\' WHERE aid = \''.(int)$afiliado.'\'')) {
    ...
    */
    function urlOut(){
        global $tsCore;
        //
		$query = mysql_query('SELECT a_url,a_sid FROM w_afiliados WHERE aid = \''.intval($_GET['ref']).'\' LIMIT 1');
        $data = mysql_fetch_assoc($query);
        
        //
        if(isset($data['a_url'])){
			mysql_query('UPDATE `w_afiliados` SET a_hits_out = a_hits_out + 1 WHERE aid = \''.intval($_GET['ref']).'\'');
            // Y REDIRECCIONAMOS
    ...
    */
    function urlIn(){
        global $tsCore;
        //
        $ref = $tsCore->setSecure(is_numeric($_GET['ref']));
		
        if($ref > 0) mysql_query('UPDATE `w_afiliados` SET a_hits_in = a_hits_in + 1 WHERE  aid = \''.intval($_GET['ref']).'\'');
        // 
