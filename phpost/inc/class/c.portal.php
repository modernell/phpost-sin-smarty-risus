...
class tsPortal{
     ...
     */
     public function savePostsConfig(){
        ...
        if(mysql_query('UPDATE `u_portal` SET `last_posts_cats` = \''.$tsCore->setSecure($cat_ids).'\' WHERE `user_id` = \''.$tsUser->uid.'\'')) return '1: Tus cambios fueron aplicados.';
     ...
     */
     public function composeCategories(){
        ...
        //
        $query = mysql_query('SELECT `last_posts_cats` FROM `u_portal` WHERE `user_id` = \''.$tsUser->uid.'\'');
        $data = mysql_fetch_assoc($query);
        $data = unserialize($data['last_posts_cats']);
        if (is_array($data)) // FIX pass empty array into home
        {    
                //                
     ...
     */
     public function getMyPosts(){
        ...
        if(is_array($cat_ids)){
            ...
            //
            $query = mysql_query('SELECT COUNT(p.post_id) AS total FROM p_posts AS p WHERE p.post_status = \'0\' AND '.$where.'');
	...
	function getLastPosts($type = 'visited'){
		global $tsUser;
        //
       $query = mysql_query('SELECT `last_posts_'.$type.'` FROM `u_portal` WHERE `user_id` = \''.$tsUser->uid.'\' LIMIT 1');
        ...
        foreach($visited as $key => $id){
            $query = mysql_query('SELECT p.post_id, p.post_user, p.post_category, p.post_title, p.post_date, p.post_puntos, p.post_private, u.user_name, c.c_nombre, c.c_seo, c.c_img FROM p_posts AS p LEFT JOIN u_miembros AS u ON p.post_user = u.user_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE p.post_status = 0 AND p.post_id = '.$id.' LIMIT 1');
            $data[] = mysql_fetch_assoc($query);
     ...
     */
     public function getFavorites(){
        global $tsCore, $tsUser;
        //
        $query = mysql_query('SELECT COUNT(fav_id) AS total FROM `p_favoritos` WHERE `fav_user` = \''.$tsUser->uid.'\'');
        $total = mysql_fetch_assoc($query);
