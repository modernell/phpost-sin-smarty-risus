...
class tsDrafts {
	...
	*/
	function getDrafts(){
		global $tsCore, $tsUser;
		//
		$query = mysql_query('SELECT c.c_nombre, c.c_seo, c.c_img, b.bid, b.b_title, b.b_date, b.b_status, b.b_causa FROM p_categorias AS c LEFT JOIN p_borradores AS b ON c.cid = b.b_category WHERE b.b_user = \''.$tsUser->info['user_id'].'\' ORDER BY b.b_date');
		//
	...
	*/
	function getDraft($status = 1){
        ...
        $query = mysql_query('SELECT bid, b_user, b_date, b_title, b_body, b_tags, b_category, b_private, b_block_comments, b_sponsored, b_sticky, b_smileys, b_post_id, b_status, b_causa FROM `p_borradores` WHERE `bid` = \''.(int)$bid.'\' AND `b_user` = \''.$tsUser->info['user_id'].'\' AND b_status = \''.$status.'\' LIMIT 1');
		//
