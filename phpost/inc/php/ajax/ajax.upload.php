...
\*********************************/
	...
	switch($action){
        ...
        case 'upload-crop':
			...
			// PARA EL PERFIL
			$total = mysql_fetch_assoc(mysql_query('SELECT p_total FROM u_perfil WHERE user_id = \''.$tsUser->uid.'\' LIMIT 1'));
			$total = unserialize($total['p_total']);
			$total[5] = 1;
			$total = serialize($total);
			mysql_query('UPDATE u_perfil SET p_avatar = \'1\', p_total = \''.$total.'\' WHERE user_id = \''.$tsUser->uid.'\'');
            // -->
        break;
		case 'upload-images':
