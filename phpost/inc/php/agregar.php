	...
	if(is_numeric($action)){
		...
		if(!empty($_POST['titulo']))
		  ...
		  $post_save = $tsPosts->savePost();
			if($post_save == 1) {
				$tsPost = $tsCore->setSecure($_GET['pid']);
				$tsCat = $tsCore->setSecure($_POST['categoria']);
				$query = mysql_query("SELECT c.c_seo FROM p_categorias AS c WHERE c.cid = $tsCat LIMIT 1");
				$tsCat = mysql_fetch_assoc($query);
				...
				$tsCore->redirectTo($post_url);
                        ...
                        else 
                        {
                            ...
                            $tsAviso=array('titulo' => 'Oops!', 'mensaje' => $post_save, 'but' => 'Volver', 'link' => 'javascript:history.go(-1)');
			}
		// EDITAR
		} 
                else 
                {
                    ...
                    $tsAviso=array('titulo' => 'Opps...', 'mensaje' => $draft, 'but' => 'Ir a pagina principal', 'link' => "{$tsCore->settings['url']}");
                    }
                    else 
                    {    
		...
		if($tsPost > 0) {
			...
			$smarty->assign("tsAviso",array('titulo' => 'Bien!', 'mensaje' => 'El post <b>'.$_POST['titulo'].'</b> fue agregado. '.(!$tsUser->is_admod && ($tsUser->permisos['gorpap'] == true || $tsCore->settings['c_desapprove_posts'] == 1) ? 'Deber&aacute; esperar su aprobaci&oacute;n' : '').' ', 'but' => 'Acceder al post', 'link' => "{$tsCore->settings['url']}/posts/{$tsCat['c_seo']}/$tsPost/{$tsCore->setSEO($_POST['titulo'])}.html"));
                        $tsAviso=array('titulo' => 'Bien!', 'mensaje' => 'El post <b>'.$_POST['titulo'].'</b> fue agregado. '.(!$tsUser->is_admod && ($tsUser->permisos['gorpap'] == true || $tsCore->settings['c_desapprove_posts'] == 1) ? 'Deber&aacute; esperar su aprobaci&oacute;n' : '').' ', 'but' => 'Acceder al post', 'link' => "{$tsCore->settings['url']}/posts/{$tsCat['c_seo']}/$tsPost/{$tsCore->setSEO($_POST['titulo'])}.html");
		} elseif($tsPost == -1){
			$smarty->assign("tsAviso",array('titulo' => 'Anti Flood', 'mensaje' => "No puedes realizar tantas acciones en tan poco tiempo. Vuelve a intentarlo en unos instantes.", 'but' => 'Volver', 'link' => "javascript:history.go(-1)"));
                        $tsAviso=array('titulo' => 'Anti Flood', 'mensaje' => "No puedes realizar tantas acciones en tan poco tiempo. Vuelve a intentarlo en unos instantes.", 'but' => 'Volver', 'link' => "javascript:history.go(-1)");
		} else {
			$smarty->assign("tsAviso",array('titulo' => 'Oops!', 'mensaje' => "Ha ocurrido un error intentalo m&aacute;s tarde.<br><b>Error</b>: ".$tsPost, 'but' => 'Volver', 'link' => 'javascript:history.go(-1)'));
                        $tsAviso=array('titulo' => 'Oops!', 'mensaje' => "Ha ocurrido un error intentalo m&aacute;s tarde.<br><b>Error</b>: ".$tsPost, 'but' => 'Volver', 'link' => 'javascript:history.go(-1)');
		}
	}
