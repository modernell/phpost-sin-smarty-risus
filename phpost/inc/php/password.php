...
/*++++++++ = ++++++++*/
	...
	$tsData = mysql_query('SELECT user_id, user_name, user_email FROM u_miembros WHERE user_email = \''.$email.'\'') or die(mysql_error());
	// borrar viejos
	mysql_query('DELETE FROM `w_contacts` WHERE `time` < \''.(time() - 86400).'\'') or die(mysql_error());
	// EXISTE?
	if(!mysql_num_rows($tsData)){
		$tsAjax = 0;
	...
	// hash
	$hash = mysql_query('SELECT * FROM w_contacts WHERE hash = \''.$tsCore->setSecure($key).'\' AND user_email = \''.$email.'\' AND type = '.(int)$type.' ORDER BY id DESC LIMIT 1') or die(mysql_error());
	if(!mysql_num_rows($hash)){
		$tsAjax = 0;
	...
	//
	if($tsContinue){
	$data = mysql_fetch_assoc($tsData);
	if($type == 2){
			if(mysql_query('UPDATE u_miembros SET user_activo = \'1\' WHERE user_id = \''.$data['user_id'].'\'') or die(mysql_error())) {
			mysql_query('DELETE FROM w_contacts WHERE user_id = \''.$data['user_id'].'\'') or die(mysql_error());
		...
		if($_POST){
			...
			mysql_query('UPDATE u_miembros SET user_password = \''.md5(md5($_POST['pass']).strtolower($data['user_name'])).'\' WHERE user_id = \''.$data['user_id'].'\'') or die(mysql_error());
			mysql_query('DELETE FROM w_contacts WHERE user_id = \''.$data['user_id'].'\'') or die(mysql_error());
