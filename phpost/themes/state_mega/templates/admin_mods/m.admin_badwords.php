                                ...
                                {if $tsSave}
                                <div style="display: block;" class="mensajes ok">Tus cambios han sido guardados.</div>{/if}
                                {if $tsError}
                                ...
                                <table cellpadding="0" cellspacing="0" border="0" width="100%" align="center" class="admin_table">
                                        ...
                                        <tbody>{foreach from=$tsBadWords.data item=b}
                                        	<tr id="wid_{$b.wid}">
                                                ...
                                                <td>{$b.word}</td>
                                                <td>{if $b.type == 1}
                                                    <img src="{$b.swop}" style="max-width:32px; max-height:32px;"/>
                                                    {else}{$b.swop}{/if}
                                                </td>
                                                <td>{$b.reason}</td>
                                                <td>
                                                    <a href="{$tsConfig.url}/perfil/{$b.user_name}" class="hovercard" uid="{$b.user_id}">{$b.user_name}</a>
                                                </td>
                                                <td>{$b.date|hace}</td>
						<td class="admin_actions">
                                                    <a href="{$tsConfig.url}/admin/badwords?act=editar&id={$b.wid}">
                                                        <img src="{$tsConfig.default}/images/icons/editar.png" title="Editar" />
                                                    </a>
                                                    <a href="#" onclick="admin.badwords.borrar({$b.wid}); return false">
                                                        <img src="{$tsConfig.url}/themes/default/images/icons/close.png" title="Eliminar"/>
                                                    </a>
                                                </td>
                                            </tr>{/foreach}
                                        </tbody>
					...
					<legend>{if $tsAct == 'editar'}Editar{else}Agregar{/if} filtro de palabra</legend>
						...
						<dl>
                                                    <dt><label for="bw_method">M&eacute;todo:</label></dt>
                                                        <dd>
                                                            <label>
                                                                <input name="method" type="radio" id="bw_method" value="0" {if $tsBW.method == 0}checked="checked"{/if} class="radio"/> Parcial
                                                            </label>
                                                            <label>
                                                                <input name="method" type="radio" id="bw_method" value="1" {if $tsBW.method == 1}checked="checked"{/if} class="radio"/> Exacto
                                                            </label>
                                                        </dd>
                                                </dl>
						</dl>
						...
						<dl>
                                                    <dt><label for="bw_type">Tipo:</label></dt>
                                                        <dd>
                                                            <label>
                                                                <input name="type" type="radio" id="bw_type" value="0" {if $tsBW.type == 0}checked{/if} class="radio"/> Texto</label>
                                                            <label>
                                                                <input name="type" type="radio" id="bw_type" value="1" {if $tsBW.type == 1}checked{/if} class="radio"/> Smiley</label>
                                                        </dd>
                                                </dl>
						</dl>
                                            {if $tsAct == 'nuevo'}
