		<div class="subMenuContent">
        	<div id="subMenuPosts" class="subMenu {if $tsPage != 'tops'}here{/if}">
                <ul class="floatL tabsMenu">
                    <li{if $tsPage == 'home' || $tsPage == 'portal'} class="here"{/if}>
                        <a class=vctip  title="Inicio" href="{$tsConfig.url}/{if $tsPage == 'home' || $tsPage == 'posts'}posts/{/if}">Inicio</a></li>
                    <li{if $tsPage == 'buscador'} class="here"{/if}>
                        <a class=vctip title="Buscador" href="{$tsConfig.url}/buscador/">Buscador</a></li>
                    {if $tsUser->is_member}
                        {if $tsUser->is_admod || $tsUser->permisos.gopp}
                            <li{if $tsSubmenu == 'agregar'} class="here"{/if}>
                            <a class=vctip title="Agregar Post" href="{$tsConfig.url}/agregar/">Agregar Post</a>
                            </li>
                        {/if}
                        <li class="{if $tsPage == 'mod-history'}here{/if}">
                        <a class=vctip title="Historial de Moderaci&oacute;n" href="{$tsConfig.url}/mod-history/">Historial</a></li>
        	        {if $tsUser->is_admod || $tsUser->permisos.moacp}
                                <li class="{if $tsPage == 'moderacion'}here{/if}">
                                <a class=vctip title="Panel de Moderador" href="{$tsConfig.url}/moderacion/">Moderaci&oacute;n 
                            {if $tsConfig.c_see_mod && $tsConfig.novemods.total}
                                <span class="cadGe cadGe_{if $tsConfig.novemods.total < 10}green{elseif $tsConfig.novemods.total < 30}purple{else}red{/if}" style="position:relative;">{$tsConfig.novemods.total}</span>
                            {/if}
                                </a>
                                </li>
                            {/if}
