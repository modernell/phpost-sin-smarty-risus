                	<div class="post-autor vcard">
                        ...
                        <div class="box_cuerpo">
                            ...
                            <li class="rango_perfil_user "align="center" style="margin-top:-20px;border:1px dashed #CCC;padding:4px;position:relative;color:#{$tsInfo.stats.r_color};background-color:#FFF">
									<strong style="color:#228194">@{$tsAutor.user_name}</strong><br>
									<span style= "font-weight:bold;color:#{$tsAutor.rango.r_color}" class="title">{$tsAutor.rango.r_name}</span>
								</li>
                            <br />
                            <br />
				          <div align="center" class="info2">
                            ...
                            {if $tsAutor.user_id != $tsUser->uid}<a href="#" onclick="{if !$tsUser->is_member}registro_load_form();{else}mensaje.nuevo('{$tsAutor.user_name}','','','');{/if}return false"><img title="Enviar mensaje privado" src="{$tsConfig.images}/icon-mensajes-recibidos.gif"/></a>{/if}                            </div>
                            ...
                            {if $tsUser->is_admod || $tsUser->permisos.modu || $tsUser->permisos.mosu}
                                    ...
                                    <div class="mod-actions">
                                        ...
                                        {if $tsAutor.user_id != $tsUser->uid}
                                            <a href="#" onclick="mod.users.action({$tsAutor.user_id}, 'aviso', false); return false;" class="alert">Enviar Aviso</a>
                                        {/if}
                                        ...
                                        {if $tsAutor.user_id != $tsUser->uid && $tsUser->is_admod || $tsUser->permisos.modu || $tsUser->permisos.mosu}
                                            {if $tsAutor.user_baneado}
                                                {if $tsUser->is_admod || $tsUser->permisos.modu}
                                                    <a href="#" onclick="mod.reboot({$tsAutor.user_id}, 'users', 'unban', false); $(this).remove(); return false;" class="unban">Desuspender Usuario</a>
                                                {/if}
                                            {else}
                                                {if $tsUser->is_admod || $tsUser->permisos.mosu}
                                                    <a href="#" onclick="mod.users.action({$tsAutor.user_id}, 'ban', false); $(this).remove(); return false;" class="ban">Suspender Usuario</a>
                                                {/if}
                                            {/if}
                                        {/if}
                                    </div>
